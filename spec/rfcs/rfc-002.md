...
```typescript
{
  ...
  // Should reference portable format values from RFC-001 Change C.
  // A Theme may target multiple formats; implementations apply it only when
  // DocumentView.format is in this list.
...
| `{{heading-1}}` | `documentWrapper`, `coverPage`, preamble | Heading prefix at level `1 + depthOffset` (RFC-001) |
| `{{heading-2}}` | `sectionWrapper`, `sectionWrapperOverrides`, preamble | Heading prefix at level `2 + depthOffset` |
| `{{heading-3}}` | `recordWrapper`, `recordWrapperOverrides` | Heading prefix at level `3 + depthOffset` |
...
2. **Should `Theme.targets` be required or optional?** Making it optional would mean the theme applies to all formats, which could produce broken output (e.g. HTML-specific templates in a Markdown render). Requiring it prevents this but adds authoring friction for themes that genuinely work across formats.

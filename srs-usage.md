...
Do not guess field names from filenames. Always resolve the Type's effective field set from `srs type get` or `srs field list`. `fieldValues` keys are `Field.name` verbatim (RFC-039); the UUID `fieldId` remains the identity of the definition in the package and its assignments.
...
The transition is validated against the lifecycle definition: the target state must exist and the transition must be allowed from the current state. A rejected transition returns `"ok": false` with a `diagnostics` entry.

**Retrofit entry (srs-rust#881, srs#447)**: a record with no `lifecycleState` at all — either freshly created before its Type carried a `lifecycleRef`, or any other record that has never been transitioned — has `currentState: ""` (see `record allowed-transitions`). From that empty state, `record transition` treats the Lifecycle's `initialState` as reachable in zero hops, and any state reachable from `initialState` by a normal chain of edges as reachable too — so `{"to": "<any-legally-reachable-state>"}` succeeds in one call even when that state is several edges past `initialState` (e.g. jumping straight to `accepted` when the chain is `draft → proposed → accepted`). This is exactly the batch-migration path: binding a `lifecycleRef` to a Type with pre-existing instances, then transitioning each one directly to its true current state in a single call, without walking every intermediate transition by hand. A target unreachable from `initialState` by any chain of edges is still rejected, same as any other illegal jump. RFC-022 `requiresRelation` obligations are enforced exactly as normal on this path too — entering a `requiresRelation` state from empty state still needs `fulfillment.newRecord`/`fulfillment.existingInstanceId`, or the relation asserted beforehand.
...
```bash
echo '{"instanceIds": ["<uuid1>", "<uuid2>"]}' | srs attachment resolve-view-attachments --repo <path> [--pretty]
```

Only tier-2 Records that have at least one `sourceRef` with `sourceRole: "attaches"` and `sourceType: "repository-document"` appear in the output. Records with no such refs are omitted.

```json
{
  "ok": true,
  "command": "attachment resolve-view-attachments",
  "payload": {
    "sourceDocumentsPath": "source-documents",
    "records": [
      {
        ...
        "attachments": [
          {
            "documentId": "4c2d9057-3ef5-40ad-a7d0-0a791b1cc782",
            "contentPath": "source-documents/transcript.pdf",
            "sidecarPath": "source-documents/transcript.pdf.meta.json",
            "title": "Phase 1 Transcript",
            "contentChecksum": "sha256:abcdef...",
            "sidecarChecksum": "sha256:123456..."
          }
        ]
      }
    ]
...
A **rejected write is a tool-level result** (`isError: true` with the service diagnostics as text), not a protocol error — agents read the diagnostics and correct the input, exactly as with the CLI envelope.

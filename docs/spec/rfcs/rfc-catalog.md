...
Evidence drove three design corrections against the issue's own option appraisal. Keying discovery on declared package boundaries loses 7 of 375 instances, which sit beside an undeclared sixth package manifest, so the rule keys on the presence of a conforming manifest instead. Requiring `$schema` would invalidate 29 conforming Records, and declaring it does not imply conformance. Retiring `sourceDocumentIndex` costs zero tombstones — there are none — though it does make one sidecar'd-but-unindexed source document a member, which is flagged as a migration audit item rather than allowed to happen silently.

An independent implementation of the proposed discovery rule returns exactly the 375 instances the existing index lists — zero discovered-but-unindexed, zero indexed-but-undiscoverable, zero path disagreements — so no instance moves and the migration is placement-only for instances. Full text: rfcs/rfc-038-tree-authoritative-storage.md.

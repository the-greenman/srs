...
**Affected Components**: docs/schema/2.0/field.json (valueType → fieldType model, Changes A–D/F); docs/schema/2.0/type.json (RequiresRelation.relationType → list, Change F; FieldAssignment cardinality props deprecated pending #242; Rev-7 conditional-required predicate description); docs/schema/2.0/package-bundle.json (embedded Field parity); docs/schema/2.0/theme.json and document-view.json (Rev-7 extension-rule predicates). RFC-032 Rev 7 also amends RFC-012 [R8]/I-120 Text Projection, RFC-019 [R6]/I-94 conditional-required, ext:themes-l1 [T-9], and ext:views-l2 [N+1], including the transition-only FieldAssignment.repeatable conjunct and its atomic Phase-B removal gate. Tier-1 TypedField.valueType stays unchanged until its explicit #242 Phase-B disposition. Rev 8 (srs#729): [R3] widened from string-only to string-or-integer for valueDomain, matching the implementation (srs#534) and the spec repo's own `com.semanticops.srs/tier` field; docs/schema/2.0/projection-rules.md's mapping table updated to match, no schema JSON changed.
...
**Title**: RFC-038: Tree-authoritative repositories and conflict-free Git storage
...
Amends standing rules and migration obligations across four Accepted RFCs: RFC-039 [R13]/[R14], RFC-026 [R5]/[R6]/[R13] and its migration steps, RFC-013 [R6]/[R9], and RFC-017 [R2]/[R12]. Establishes version discrimination via `dataModelRevision` plus an `srsj` bump to "2", closing a silent-empty-repository failure mode. Composed with RFC-039 in one first-party cutover at a single `dataModelRevision: 2`; neither ships alone. Revision 5 measures `muSrs`: 32/32 instance parity, 21 relations and two package manifests. Revision 6 fixes the root Container. Revision 7 treats every remaining item as a migration repair: gallery is the third repository and receives a marker; the discovery and RFC-032 fixtures stay test data; revision history receives its owed schema; the namespace-specific governance package owns its 27 byte-identical definitions; and the two published governance packages join the cutover. Revision 8 closes five review findings in the Phase-B contract without reopening a decision: a third owed schema (`protocol.json`, absent while both governance packages declare a protocol, so [R24] would leave them unloadable at the generation they are stamped to); `core-bundle.srsj` joins the stamping inventory because [R21] binds packages as well as repositories; [R8]'s blanket `additionalProperties: false` conjunct is scoped to multi-member admissible sets and `relation-type.json` is closed to make it true; the relation-collection count is reconciled to 3 non-empty collections, with the empty fixture collection named as excluded; and `relation.json` is stated as 17 properties / 5 required throughout. Revision 9 records the owner's disposition of Phase 0 step 2, the single item Revision 8 left open: `spec/srs-purpose-and-scope.md` is moved out of `sourceDocumentsPath` rather than adopted, so the cutover adds no source-document membership anywhere and `srs/`'s source-document set stays at the 4 already-indexed documents; this reopens nothing and adds no policy choice of its own.
...
Because membership is recorded centrally, every write that adds an instance edits the same file. `srs/manifest.json` is 95,846 bytes across 3,560 lines indexing 375 instances; adding one Tier-0 Note writes the note and then edits the file every other concurrent writer is editing. PR #291 is the reproduction: six independent notes, unique paths, no semantic overlap, conflicting PR. Relations have the same shape — 243 across 3 non-empty live collections in the full migration scope, 205 of them in one `srs/` file.

RFC-038 makes the authoritative store answer membership and the manifest a stable descriptor. Its substance is the enumeration contract, because RFC-013 [R2] defines membership as "the union of the instance files declared by the repository" without ever saying how a repository declares them: remove the index without defining enumeration and membership becomes undefined for the first time.
...
**Affected Components**: The com.semanticops.spec authoring package (srs/package/spec-authoring-core/): section and subsection retire as authored structure; a new mechanism Type; concept and example revived as carrying Types; invariant.applies_to retires; the spec repository's compositions, root container and Part containers; RFC-016 [R6] (index grouping, superseded here); the scripts/check-spec-coherence.mjs contract (srs#560); the normativeSites entry of scripts/spec-language-registry.json. Revision 5 (srs#796, executing rfc-decision-8aed3412) amends Change G and [R13] on this RFC's own surface: a declared child container renders as a nested section under containerScope subtree on the container-subset source, and the flat-Composition limit Revision 2 recorded is lifted. Two additive, default-preserving schema changes: containerScope on the container-subset SectionSource (composition.json) and recursive sections on ProjectedSection (document-view-output.json). No dataModelRevision change (stays at 7). cell:portability joins the cell set at Revision 5. Canonical folding executes as the Wave 3 units srs#559 (mechanism Type + exposition_role/layer/relations), srs#562 (un-bake 127 headings into records), srs#563 (rewrite Parts I-IV reading order), srs#564 (invariants inline, generated index) — none has executed yet, so this record is grandfathered in rfcs/integration-allowlist.json pending their completion.

...
**Decision**: The `SectionSource` `type-query` variant's selection key becomes Type-keyed: a `typeKey` string in `namespace/name` form (version-independent, KEYED per rfc-decision-c8704763), resolved against the effective installed package set at query time — the same resolution `typeFilter`/`typeDispatch` already perform elsewhere in `document-view.json`. `semanticObjectType` (a bare, unscoped string with no installed-definition backing) is retired from `SectionSource` once #272 migrates every consumer; until then it stays modelled as sanctioned-until-collapsed (RFC-040 metamodel v1.1.0, #383). Every other `type-query` filter axis (lifecycleState, lifecycleStates, excludeLifecycleStates, containerIds, containerScope) is unchanged — only the selection key changes shape, from an unscoped free string to a scoped, installed-definition-backed key. A `typeKey` that does not resolve in the effective package set is a validation error at query time, not a silent empty result (matching typeFilter's existing contract) — the same fail-closed posture RFC-040 applies throughout: an unrecognised dispatch target is an error, never quietly ignored. This is a design record only: no schema file changes here. #272 owns the SectionSource schema edit, the four-site semanticObjectType retirement (type.json, view.json, relation-type.json, document-view.json), the governance package's 1.2.0 republish with migrated document-views, the srs-rust dispatch removal (SectionSource::TypeQuery, governance_scaffold_service.rs's DECISION_TYPE_KEY dispatch), the vendored-seed re-vendor, and the I-142/143/144 invariant amendments (srs#410's relocated RFC-011 invariants governing this exact SectionSource shape).

**Scope**: Governs the eventual SectionSource type-query redesign and nothing else; does not itself change any schema, package, or Rust code. Placement per the 2026-08-15 owner ruling on srs#383 and the 2026-08-24 #273 planning session's routing comment. Supersedes no accepted RFC; narrows no standing contract. #272 is execution-ready to consume this design directly.
...
**Alternatives Considered**: Keep semanticObjectType as the permanent dispatch key (rejected: the owner's one-way-per-goal ruling already found this a parallel taxonomy that must collapse into the Type system, srs-rust#826/#383). PINNED {typeId, typeVersion} dispatch (rejected: rfc-decision-c8704763's axis 2-8 corollary — presentation/dispatch must not absorb a version bump; a DocumentSection authored against one Type version would silently stop matching Records of a newer version). A fifth reference strength invented specifically for this case (rejected: c8704763 states no fifth strength without a charter amendment, and KEYED already covers this shape with existing precedent in the same schema).

**Accepted Costs**: Deferred execution risk: this design is not enforced by any schema or test until #272 lands it, so it could in principle drift from what #272 actually builds if the two are separated by a long interval — mitigated by this record being the cited, execution-ready contract #272's issue points to, not a free-floating suggestion. Authors of new DocumentSection type-query sections between now and #272's landing continue to use semanticObjectType (sanctioned-until-collapsed) since the KEYED form is not yet implemented.

**Evidence**:
- srs-rust#826 (triage that surfaced the vendored-seed divergence)
- srs#383 (execution tracker; 2026-08-15 owner ruling; 2026-08-24 placement comment)
- rfc-decision-c8704763 (the reference taxonomy; PINNED/LINEAGE/KEYED/LOCATOR strengths and the axis 2-8 corollary)
- RFC-040 Change K (train unit 4a-5, srs#481)
- the closed #163 (portability class the container-subset repair risked, cited on srs#383)

**Review Trigger**: #272 begins execution (SectionSource schema edit, semanticObjectType retirement, governance 1.2.0, Rust dispatch removal, re-vendor, I-142/143/144 amendments) — at which point this record's design_statement is either implemented as stated or amended with the reason it changed.
...
**Decision Rationale**: The check surface accreted piece-by-piece into four shapes for one goal (check-*.mjs assertions, gen-*.mjs --check byte-closure gates, migrate-*.mjs --check applied-gates, tests/*/run.mjs suites) plus an invisible fifth tier of checks deliberately outside validate-all (check-release-drift, which needs the pinned binary). The cost stopped being theoretical on 2026-08-26: train unit #483's agent ran validate-all, got exit 0, and shipped PR #493 red because the drift gate lives outside the surface it checked - nothing declared that the surface had a second half. In grid terms the checks are sound Conformance machinery, but the registry of checks was undeclared structure, inferred by reading a 260-line script - an Earth violation (structure is declared, never inferred) and a meta-level one-way-per-goal failure.

**Decision**: Cell: Conformance. Every spec-repo conformance check is declared in a single registry (scripts/checks.json) rather than hand-wired: each entry names the check's id, script, arguments, tier (always | pinned-cli | ci-only), the grid cell it guards, the governing decision or RFC that introduced it, and an optional expiry for checks born of grandfathering. validate-all.mjs becomes a loop over the registry filtered by tier, and it ends every run by naming the checks deferred to other tiers, so its exit 0 can never again be mistaken for the full gate surface. A membership guard enforces the registry: a check script present on disk but absent from the registry fails the build, which makes piecemeal addition structurally impossible. New checks follow one naming convention; existing script names migrate only when next touched (rename is a migration). The registry is a stepping stone, not the destination: the end state is the checks described as records - the spec repository declaring its own conformance gates the way it declares everything else, with the JSON registry generated as a projection. Owner, 2026-08-26: as long as we keep working towards the goal where all meaning is in SRS we are going in the right direction.

**Scope**: Binds how conformance checks are added to the srs spec repository from ratification: declaration in the registry precedes wiring, the membership guard enforces it, and the tier vocabulary is the one named here. Does not force-rename existing scripts, does not bind sibling repositories (their gates follow their own contracts), and does not build the checks-as-records fold now - that lands with the SRS-for-Specs work and the #490 meaning-placement pass, which this record hands the fold to.
...
**Alternatives Considered**: (1) Status quo hand-wiring - rejected by evidence: it produced four conventions and today's red PR. (2) Jumping straight to checks-as-records - deferred, not rejected: it is the declared end state, but building it now widens the current phase; the registry file is the smallest declaration that makes the later fold trivial. (3) Adopting a test-framework dependency to organise the checks - rejected: the problem is undeclared structure, not missing machinery; the fix is data, not a framework.

**Accepted Costs**: One added indirection between validate-all and the scripts. The ci-only tier can declare a check but cannot enforce that the CI workflow actually runs it - the registry makes the gap visible in review rather than closing it. Registry entries carry provenance fields that must be filled honestly at addition time.

**Evidence**:
- PR #493 (train unit #483) shipped red on check-release-drift, 2026-08-26: validate-all exit 0 while the required CI gate failed - the motivating incident
- Master inventory 2026-08-26: ~25 checks in four shapes wired as individual runScript lines in validate-all.mjs, plus check-release-drift outside it
- rfc-decision-cce3c00e (the grid; Conformance cell preference: declared strength over convenient reach)
- rfc-decision-9ee14517 (layer rules: behavior needs a contract; structure is declared, never inferred)
- srs#490 (post-256 meaning-placement review - carries the checks-as-records fold and the generator-inversion candidate this trajectory joins)
...
**Title**: A construct has one layer: a crossing is a stated goal, never a design option

**Status**: Accepted

**Decision Date**: 2026-09-06

**Decision Rationale**: The owner's ruling, made while closing the container review (srs#605): the single-layer rule was the element that mattered, and a rule that names an exception in the same breath as the rule hands every future author an option they will use. 'Legitimately' was never defined; the corpus holds zero declared crossings under rule 4, so the permission has cost nothing yet - which is exactly the moment to withdraw it. The container question flip-flopped for a year because each RFC could add one more job to the Container without any rule saying a construct may not straddle planes. Rule 1 (one home) names the plane; it did not, by itself, forbid a construct from carrying a second plane's data as a 'declared crossing'. This record closes that gap: the crossing mechanism survives for the case that genuinely needs it, and stops being the path of least resistance.

**Decision**: Layer rule 4 is refined. Its ruling clause - a crossing is declared where it occurs, and the lower layer wins - stands. Its permission clause - 'where a layer legitimately carries another layer's data' - is withdrawn as a standing option.

The rule now reads: A CONSTRUCT HAS ONE LAYER. Every construct is defined on one plane and one layer, and its schema carries only that layer's data. A layer carries another layer's data only for a specific, named goal that cannot be met within one layer. That goal is stated in the decision or RFC that introduces the crossing, the crossing is declared at the site where it occurs, and conflicts resolve downward, visibly. Absent a stated goal the layer test fails: the proposal is re-drawn inside one layer, not accepted with a declaration attached. Identity conflicts remain fatal per the Earth rule - they are not crossings.

The two examples the original rule named are re-read under this reading. Denormalized type hints on a Record (typeNamespace, typeName; Invariant 28) are a crossing with a stated goal - a record file is readable without its package present - and remain the one grandfathered crossing in the core. Composition order defaulting to the precedes chain is not a crossing at all: it is an upper layer consuming a lower one (rule 2), and is re-filed as such.

Applied to the Container (rfc-decision-0750c62f): the plane assignment is closed, not provisional. A Container is a selection and nothing else; no future consumer may add semantic data to the Container schema by declaring the addition a crossing. A consumer that needs meaning attached to a scope attaches it to the scope's identity record, where Fields live.

**Scope**: Layer rule 4 of rfc-decision-9ee14517, every construct schema in the core and the extensions, and the layer test applied to every proposal. Does not change rules 1, 2, 3, 5 or 6. Does not alter Invariant 28. Does not change any schema on disk today.
...
**Alternatives Considered**: (1) Leave rule 4 as written and rely on review to judge 'legitimately' case by case. Rejected: an undefined word in a rule is a preference, not a rule, and the container history shows how a preference erodes one RFC at a time. (2) Forbid crossings outright and retire Invariant 28's hints. Rejected: the hints serve a real goal (a readable record file without its package) and removing them is a breaking change with no consumer asking for it; a goal-gated exception keeps the one real case and closes the door on speculative ones. (3) Move the tightening into the Container decision alone. Rejected: the failure is general - any construct on any plane could grow a second plane's data by declaration - so the fix belongs in the layer register, and the Container ruling becomes its first application.

**Accepted Costs**: Some conveniences that would have passed as declared crossings must now be re-drawn inside one layer or justified by a stated goal; that is more design work up front. A future construct with a genuine cross-layer goal must argue it explicitly rather than assert it. Any existing spec surface that quietly carries another layer's data without a declaration is now a finding, not a tolerated convenience - the meaning-placement sweep (srs#588 lineage) gains a new class of item.

**Evidence**:
- rfc-decision-9ee14517 (rule 4 as originally worded)
- rfc-decision-0750c62f (the container ruling this closes)
- srs#605 (the container review, owner's closing ruling on the single-layer element)
- Invariant 28 (the one grandfathered crossing)
- corpus search 2026-09-06: zero declared crossings under rule 4 outside the charter itself
...
**Title**: Retirement rules speak at two levels; a Type elimination deletes retired instances once provenance is recorded

**Status**: Accepted

**Decision Date**: 2026-09-10

**Decision Rationale**: srs#710's programme-unit-walk Protocol found, at Ground/Charter stage, that the established section/subsection-to-concept migration pattern (derived-from successor, no deletion) can never satisfy that issue's own acceptance criterion (record list --type returning zero), because record list does not filter by lifecycleState and rfc-decision-5f8204bc's instance clause rules that Records retire by supersession, not disappearance. That is exactly the review_trigger 5f8204bc names: a retirement case fitting none of the three layer mechanisms it defines, because a whole-Type elimination is a one-time structural change, not the routine content churn supersession assumes. The owner's ruling resolves it by naming a distinction the record's text did not make explicit: 5f8204bc rules a repository's own governance, not the standard's mechanisms.

**Decision**: Retirement rules exist at two levels, and rfc-decision-5f8204bc's instance clause was written at the wrong one without saying so.

The standard and its tooling provide mechanisms for retiring instances - the supersedes, refines, and derived-from relation types, lifecycle states, and record delete - and enforce nothing beyond honesty about what each mechanism does. Provenance (a successor recording what it replaced) is a capability the tool must make achievable; it is not a mandate the tool imposes on every repository that uses it. A generic tool defines what is possible, not what every corpus must do with it.

rfc-decision-5f8204bc's instance clause reads, verbatim: "INSTANCES (Records) retire by SUPERSESSION - successor-and-link (supersedes/refines), per the Fire column principle; history stays addressable." That sentence is repository-governance policy, adopted by governed corpora such as this one - not a rule of the standard itself. A future worker or agent must not read it as binding every SRS repository's retirement behavior; it binds only corpora that adopt this charter.

The owner's ruling states the distinction directly: "we are building a generic tool. We are defining rules on two levels. We have rules for how we can maintain a consistent repo, especially one that's governance focussed. This needs rules on succession. But as a tool, we are not enforcing this, governance is just one use. Provenance is our capability goal we need to be able to achieve it, but we are also just building a generic tool."

The exception 5f8204bc's own review_trigger anticipated is now named: eliminating a Type entirely - removing its definition from the package so no Record may ever bind to it again - is a one-time structural change, not the content churn the supersession rule assumes. Under this exception, once each retiring instance's successor has recorded provenance (the successor's meta.derivedFrom field set to the original instanceId, so the paper trail survives the retiring record's own deletion), the retiring instance is deleted through the CLI, not merely superseded-and-kept. Git history is the addressable history for a first-party corpus; it is what 5f8204bc's "history stays addressable" clause can rely on once the record itself is gone. The Type's own definitions (the Type record and its Field assignments) are deleted last, only after every instance that bound to them is gone.

A consequence follows for how completion is measured: record list --type showing historical entries for a Type that still has live instances is expected behavior, not a defect - a list is a list, and lifecycleState is not a list filter. For a genuine Type-elimination unit, the acceptance criterion is that the Type's definitions no longer appear in type list and that repo validate reports zero errors - not that record list --type returns zero while instances of that type still exist.

This exception does not relax the honesty requirement: record delete's relation cascade currently removes inbound relations (including a derived-from edge) silently, with no diagnostic, which makes the provenance capability this record relies on invisible to whoever runs the delete. srs-rust#1025 is the tooling obligation this decision creates: the delete cascade must report what it removes, so a Type-elimination unit can verify provenance survived the deletion it just performed rather than trusting that it did.

**Scope**: The instance clause of rfc-decision-5f8204bc only - the sentence quoted above and nothing else in that record. Every other clause of 5f8204bc stands exactly as ratified: DEFINITIONS (Field, Type, View, Composition, Theme, Blueprint, Protocol, Package definitions) retire by deletion with version history as the trail; SUBSTRATE ENTRIES (Terms, LifecycleStates, RelationTypeDefinitions) retire by status; the cross-layer rules (self-expiring exceptions, the deprecated:true annotation, the word 'deprecated' reserved for substrate status) are untouched. This record's level is repository governance (♐ Governance); it binds corpora that adopt the charter, not SRS implementations.
...
**Alternatives Considered**: (1) Never delete a retired instance once a successor exists - keep every retiring section/subsection Record alive indefinitely alongside its concept successor. Rejected: it makes the Type definitions themselves undeletable, since a Type cannot be removed from the package while any instance still binds to it - the whole point of a Type elimination (RFC-042 Change B) would be permanently unreachable, and record list --type <retiring-type> would never reach zero by any mechanism, ruled or not. (2) Treat this as a correction to rfc-decision-5f8204bc rather than a successor. Rejected under rfc-decision-4431046e's own test: a reader who acted on 5f8204bc's instance clause as written - assuming it bound every SRS repository - would act differently after this ruling, so it is a refinement of position, not a fix to a wrong detail, and must ship as a successor record, never an in-place edit.

**Accepted Costs**: Once a retiring instance is deleted, its provenance is recoverable only through the successor's meta.derivedFrom field and, for anything meta.derivedFrom does not itself carry, through git history - not through a live derived-from relation, which the delete cascade removes along with the deleted instance. This is accepted because git history is addressable for a first-party corpus and because srs-rust#1025 will make the cascade's removals visible at delete time, rather than eliminating the cascade or the deletion itself.

**Evidence**:
- srs#710 (comment 2026-09-10, the parking finding and the owner's ruling)
- rfc-decision-5f8204bc (the record refined; its instance clause and review_trigger)
- rfc-decision-4431046e (the correction-versus-refinement test this record is authored under)
- rfc-decision-5f18603e (the precedent successor record: quotes the text it changes, scopes to one clause)
- srs-rust#1025 (delete-cascade visibility, the tooling obligation this decision creates)
...
**Title**: The .srs repository marker is a directory; its non-emptiness convention is spec, the satisfying file is implementation-owned

**Status**: Accepted

**Decision Date**: 2026-09-16

**Decision Rationale**: srs#313 (closed 2026-08-02) found the spec and the implementation disagreeing about the `.srs` marker's kind: the spec's prose called it a file ("empty, or containing a format version on its first line"), while RFC-038, `srs-usage.md`, and every srs-rust code path (`detect.rs find_repo_root`, `FileStore::repository_exists` via `is_dir`, the `.srs/.gitkeep` emission in `tree_session.rs`/`archive.rs`/`srsj.rs`) already agreed it was a directory. The owner's ruling on that issue, made in comments on 2026-08-02, had four numbered parts: (1) the marker is a directory — Invariant 45 and the spec prose (`srs-spec.md:2491`) were the sites that needed to change; RFC-038 [R5] and `repository_exists()`'s `is_dir` already agreed and stood. (2) Non-emptiness is a *convention*, not a normative rule justified by git: the empty-directory loss is a VCS limitation — an implementation concern, not a property of the SRS format — so the spec does not encode around it; the convention stands on its own terms. (3) The default member is an "About SRS" README, whose purpose is discovery and LLM orientation for a reader landing cold in the repository — not merely keeping the directory alive under git; surviving git is a side effect of the file being worth having, not its justification. (4) Drafting that README is deferred to its own task (srs#340, transferred to srs-rust#1066).

Commit 88323e2 folded part (1) into Invariant 45 and the `ext:repository` mechanism the same day. Parts (2)-(4) were never folded into a `rfc-decision-*` record — the ruling existed only in the srs#313 issue thread, with no home in the decision log `srs/srs-usage.md` and the compass point to. This record closes that gap retroactively: it is recording a ruling made on 2026-08-02, not making a new one.

A first draft of this record got part (2) wrong: it justified the README by the git round-trip argument the owner had already rejected in the same thread (the reasoning that sank `.gitkeep` as a candidate: "a git artifact leaking into a format specification"). The owner corrected this on 2026-09-16. The convention stands on discovery/orientation value (part 3), not VCS accommodation; and *which* file discharges it is an implementation choice srs-rust can change independently, tracked at srs-rust#1066. Naming `.srs/README.md` specifically in a spec decision record would also over-specify: under the convention as actually ruled, no new file is required everywhere — the spec repo's own `.srs/profiles/foundation.json` already satisfies non-emptiness, and binding the spec to a filename would make that already-conforming repository non-conformant until it grew a file it does not need.
...
(2) The marker directory's non-emptiness is a convention (SHOULD), not a MUST justified by git. An emptied `.srs/` does not fail silently: `repository_exists()`'s `is_dir` check returns false and the tool reports "not an SRS repository" at first use — a loud but misdiagnosable failure (a documentation problem, not a conformance problem), which is why this is a convention rather than a hard rule. The spec does not encode a VCS's limitation (git drops empty directories) as a property of the SRS format; the convention stands on its own terms, independent of git.

(3) The convention's default member is an "About SRS" orientation document, whose purpose is discovery and LLM orientation for a reader who lands cold in the repository — not merely keeping the directory alive under git (that survival is a side effect of the file being worth having, not its justification).

(4) *Which* file discharges the convention — its name, its contents, and the command that generates it — is delegated to the implementation and is not decided by this record. Drafting the orientation document is its own deferred task, currently srs-rust#1066 (transferred from srs#340), proposing `.srs/README.md` to replace the `.gitkeep` placeholder `repo create` emits today — `.gitkeep` was itself considered and rejected on srs#313 as "a git artifact leaking into a format specification." `.srs/` contents remain implementation-private and outside conformance per RFC-038 [R5] — the repository walk skips `.srs/`, so nothing inside it, whatever file is chosen, can trip [R24] fatality or carry a conformance rule, a schema element, or a semantic claim (CC-43 names this drift-to-normative hazard; this boundary is the standing mitigation). This record does not name a file: under the convention as ruled, `srs/.srs/profiles/foundation.json` already satisfies non-emptiness for the spec repo itself, which needs no README to comply.

**Scope**: Governs the `.srs` marker's kind (directory, part 1), the convention-not-MUST status of its non-emptiness and the reasoning for that status (part 2), the discovery/orientation purpose of its conventional default member (part 3), and the delegation of selecting and producing that member to the implementation (part 4) — across every SRS repository (spec, srs-rust, srs-vscode, and any conforming corpus). Does not name a file, does not author one, and does not reopen RFC-038 [R5] or Invariant 45. Does not decide srs-rust#1066's own design — that stays implementation-owned and owner-merge gated there.
...
**Alternatives Considered**: Migrate the implementation to a marker *file* instead — the alternative srs#313 itself named. Rejected: every implementation code path already treated the marker as a directory before the spec was consulted (RFC-038 [R5] predates this ruling); the corpus's disagreement was the spec being wrong, not the implementation.

Make non-emptiness a MUST rather than a SHOULD — considered, since an emptied `.srs/` does make `repository_exists()` return false. Rejected: the failure is loud (the tool reports "not an SRS repository"; it does not silently misbehave) but misdiagnosable — a documentation/tooling problem, not a conformance violation to enforce at the format level. Encoding it as a hard rule would also mean encoding around one VCS's limitation (git drops empty directories), which the spec should not assume every SRS repository even uses.

Name `.gitkeep` as the convention's satisfying file — the status quo `repo create` still emits. Rejected on srs#313 itself: "a git artifact leaking into a format specification. SRS repositories need not live in git, and the spec should not assume they do." `.gitkeep` is not treated here as a neutral baseline the README merely improves on; it is a rejected form still shipping, tracked for replacement at srs-rust#1066.

Name `.srs/README.md` specifically in this decision record — an earlier draft of this record did exactly that, and the owner identified it as a second instance of the same over-specification (2026-09-16). Rejected: the spec's job is to state the convention and its purpose (discovery/orientation, part 3), not pick a filename the implementation is free to change; the spec repo's own `.srs/profiles/foundation.json` already satisfies non-emptiness today, so binding the spec to `README.md` would make an already-conforming repository non-conformant until it grew a file it does not need. Recording a filename would also recreate exactly the coupling this ruling avoids: a later srs-rust rename would require amending a spec decision record rather than closing an implementation issue.

Doing nothing (leaving parts (2)-(4) of the srs#313 ruling unrecorded, as they were for six weeks) was also live and is what actually happened until now — rejected because it is the gap this record closes.

**Accepted Costs**: The convention is a SHOULD, not a MUST: `repo validate` cannot detect an emptied `.srs/` as a conformance error, because by the time `repository_exists()`'s `is_dir` check fails the directory no longer reads as a repository at all — the failure surfaces as "not an SRS repository", not as a diagnostic, which can misdirect debugging toward "this isn't a repo" rather than "this repo lost its marker's placeholder". This record accepts that cost rather than promoting the convention to a MUST, per part (2)'s reasoning. It also accepts that, until srs-rust#1066 lands, `repo create` keeps emitting the rejected `.gitkeep` placeholder rather than the oriented README the ruling calls for; this record does not close that gap, it only confirms the convention and hands the choice of file to the implementation.

**Evidence**:
- srs#313 (closed 2026-08-02) — the four-part ruling this record captures retroactively: directory kind, non-emptiness as convention (not git-justified), README as the default member for discovery/orientation, and drafting deferred; the ruling existed only in issue comments, never as a decision record, until now
- commit 88323e2 — "fix(spec): fold .srs marker directory decision into canonical spec (#313)", folding part (1) into Invariant 45 and the ext:repository mechanism, migrating docs/spec/examples/gallery-project-v2/.srs from file to directory
...
- mechanism-d453219f (Repository layout) — states the marker SHOULD contain a regular file so it survives round-trips; its prose currently also names `.srs/README.md` as 'by convention' — this record's part (4) treats that as illustrative only, not a spec commitment to a filename, since choosing the file is implementation-owned
- RFC-038 [R5] — codifies the directory reading; the repository walk skips `.srs/`, so [R24] fatality does not reach inside it
- CC-43 (srs#313 ruling thread's hazard ledger) — names the risk of `.srs/` contents drifting into normative weight; RFC-038 [R5]'s walk-skip is the standing mitigation this record relies on in part (4)
- srs-rust#1066 (open, gate:owner-merge, transferred from srs#340) — tracks drafting the "About SRS" orientation document that will replace `.gitkeep`, constrained to never become authoritative
- srs-rust#691 (filed 2026-07-22) — "Spec/impl divergence: .srs marker is a file in the spec, a directory in the implementation"; this ruling's part (1) resolves it
...
**Title**: The level test: a spec rule must be declarable and checkable, or it is an authoring rule

**Status**: Accepted

**Decision Date**: 2026-09-17

**Decision Rationale**: srs#812's meaning-placement sweep found the spec asserting rules that no repository can declare and no implementation can check. The owner's example is the sharpest: `contains` has exactly one parent is load-bearing (I-148, I-66, RFC-042 [R2]) and this repository obeys it perfectly across 542 edges, yet nothing in SRS made it so. Relation validation is exactly E1-E4 in crates/srs-core/src/validation/relation.rs; `repo validate` has no cardinality, tree-shape or cycle check for any relation type; `RelationTypeDefinition` exposes only `irreflexive` and `requireSameType`. The rule is held up entirely by scripts/check-spec-coherence.mjs check (2), a bespoke Node script binding this repository and no other. So a rule about how we write the spec sits in the spec as though it constrained every SRS user, and it repeats about nineteen times.

The principle that resolves this was already ruled, for one clause of one record, by rfc-decision-1e7c0c8e. It never generalised, and the compass has no section for the distinction, so each unit re-derives it from scratch - the drift shape the charter names as "a rule stated once, remaining sites silent". Design-note 044 R10 already supplies the removal half of the test ("a principle stated in prose but enforced nowhere is a defect, to be enforced or removed") but not the hold half, and the removal half alone would cut rules whose only defect is that the machinery is unbuilt. The owner's ruling states the test and adds the third disposition that keeps it from being a demolition order.

**Decision**: A normative statement belongs in the SRS specification only if some repository can DECLARE it through SRS machinery and some implementation can CHECK it. If it can only be obeyed, it is an authoring or governance rule and lives at that level - this repository's spec-language guide, or the charter. Declarable and checkable are one conjunction: a constraint nobody can state in a repository's own data is not made normative by an implementation enforcing it, and a constraint that is declarable but that no implementation reads is an affordance without a contract (layer rule 6), not a rule.

This generalises rfc-decision-1e7c0c8e from the one clause it was written for into a standing placement test. That record ruled, of rfc-decision-5f8204bc's instance clause alone, verbatim: "The standard and its tooling provide mechanisms for retiring instances ... and enforce nothing beyond honesty about what each mechanism does. ... A generic tool defines what is possible, not what every corpus must do with it." The principle was never clause-specific; only its application was. It is now a test asked of every normative statement, at Charter Check time and at review, alongside the cell check and the layer test.

A rule that fails the test has three dispositions, not two. It may be RELOCATED to the level that owns it, where it keeps its force over this repository and loses its false claim over every other. It may be RETIRED outright, where a declarable construct already does its work. Or it may be HELD. A rule is held when it is a mechanism we intend to build: it stays in the spec unenforced, and the change that holds it names the machinery that would carry it and the issue that owes it, joining the make-it-real queue. A held rule is a debt with a named payer, never an exemption; if the machinery is declined, the rule falls back to relocation or retirement. Without the hold disposition the test reads as a demolition order, and that is not what is ruled here.

This is not the sanctioned two-tier boundary that rfc-decision-43249f53 rejected, and the objection is answered rather than set aside. That record refused two tiers of one mechanism - a bare status enum for light cases, a Lifecycle when transitions matter - on the ground that "the two-tier boundary would itself become a drift seam": authors would drift across the line case by case, and the same job would be done two ways inside SRS. Nothing here is two-tiered. There is one mechanism, SRS's declarable and checkable machinery, and separately one document-authoring convention that this repository writes by and that no SRS implementation reads. They do not compete for the same job, and they do not sit in the same repository's scope: SRS neither requires nor forbids the authoring convention of anyone else, and the convention claims no conformance force over anyone, including us. The seam 43249f53 feared runs inside a single mechanism; the line drawn here is the boundary between a mechanism and a house style, which is the boundary one-way-per-goal exists to keep clean rather than a breach of it. Where the two could genuinely blur is a held rule, which by construction sits in the spec while its machinery is unbuilt - which is why a hold must name its machinery and its issue, so the blur has an end date instead of a habit.

**Scope**: The placement of normative statements on the SRS specification's normative surface - the record Types that scripts/spec-language-registry.json declares normative (com.semanticops.spec/invariant and com.semanticops.spec/mechanism) and the conformance rules of accepted RFCs. This record's own level is repository governance (Governance); it binds the authoring of this specification and corpora that adopt this charter, not SRS implementations, and it creates no load-time invariant and no new check. It does not rule the disposition of any individual existing rule: the sweep that applies the test is srs#818, the authoring guide that receives what it relocates is srs#819, and the RFC that would discharge the held rules is srs#820. rfc-decision-1e7c0c8e's own application to rfc-decision-5f8204bc's instance clause, and every scoping clause 1e7c0c8e states about 5f8204bc, stand exactly as ratified; what this record changes is the reach of the principle, not any disposition 1e7c0c8e made under it.
...
**Alternatives Considered**: (1) Leave rfc-decision-1e7c0c8e clause-scoped and apply its principle ad hoc, unit by unit. Rejected: that is the status quo, and it is what produced about nineteen restatements of one authoring rule as universal spec text; a principle with no section in the compass is re-derived (or not) by every reader, which is the drift shape the charter names. (2) State the test with two dispositions only - relocate or retire - which is what design-note 044 R10 says on its own. Rejected by the owner: it would cut rules whose only defect is that the machinery is unbuilt (RFC-042 [R2] single-parent, precedes acyclicity, the naming grammar of mechanism-b3c293f9), turning an honesty measure into a demolition order and foreclosing the very capabilities srs#820 exists to add. (3) Make the test a mechanical check rather than a review test. Rejected: "can some repository declare this" has no machine predicate today, and a guard that could be written would only re-express the reviewer's judgement as a keyword scan - the observability-pressure audit's own warning about citing a test without honouring it. (4) Treat this as a correction to rfc-decision-1e7c0c8e rather than a successor. Rejected under rfc-decision-4431046e's test: a reader acting on 1e7c0c8e as written applies it to rfc-decision-5f8204bc's instance clause and nothing else; a reader acting on this record applies it to every normative statement in the spec. The position's reach changed, so it ships as a successor record and 1e7c0c8e is never edited.

**Accepted Costs**: The test is a review test, not a guard: like the cell check and the layer test it can be cited without being honoured, and "declarable and checkable" is judged by a reviewer. No check is added here, and the charter gains a rule whose observability is the hard-to-verify kind the 2026-08-23 observability-pressure audit warns bites first. The hold disposition is the pressure point, because held is the comfortable answer for any rule someone would rather not cut; it is legitimate only with the machinery and the issue named, and a hold whose issue closes without the machinery is a finding, not a settled exemption. Relocating rules off the normative surface also means a conformant implementation is no longer told to enforce them - accepted deliberately, since no implementation could enforce them in the first place and the core conformance profile that enumerated them was therefore unsatisfiable.

**Evidence**:
- srs#812 (the unit, the owner's ruling, and the enforcement survey)
- rfc-decision-1e7c0c8e (the record generalised; the generic-tool clause quoted above)
- rfc-decision-43249f53 (the two-tier drift-seam objection this record answers)
- rfc-decision-4431046e (the correction-versus-refinement test this record is authored under)
- rfc-decision-5f18603e (the precedent successor record: quotes the text it re-levels, scopes to one clause)
- srs/records/design-notes/044 R10 (a principle enforced nowhere is a defect, to be enforced or removed)
- crates/srs-core/src/validation/relation.rs in the-greenman/srs-rust (relation validation is exactly E1-E4)
- scripts/check-spec-coherence.mjs check (2) (the repository-local check that actually holds the single-parent rule up)
- srs#818 (the sweep applying the test), srs#819 (the authoring guide that receives relocated rules), srs#820 (the RFC that would discharge the held rules)

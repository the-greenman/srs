...
| entity-level conditional co-occurrence (`allOf`/`if`/`then` for R2/R3/R9/R10) | **authoritative** (RFC-040 Unit 3, srs#479) — `schema-emitter.mjs` now emits the `FieldType` entity's `allOf` itself (`FIELD_TYPE_ENVELOPE`, hand-mirrored from `validateFieldType` R2/R3/R9/R10), matching the frozen seed | approximated | approximated | approximated | `validateFieldType` R1–R11 remains the semantic source of truth this envelope mirrors |
| cross-field validation (`CrossFieldRule`, RFC-019) | **authoritative for `conditional-required`/`conditional-forbidden`/`mutual-exclusion`** (RFC-040 Unit 3) — a Type's own `validationRules` project to real `allOf`/`if`/`then`/`not` guards (`projectValidationRules`); **approximated for `field-ordering`** — document order has no JSON Schema construct, so it is left unprojected by design | approximated | approximated | approximated | RFC-019 cross-field validation (`field-ordering` only) |

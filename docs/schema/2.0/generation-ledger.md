...
| `relation-type.json` | `RelationTypeDefinition` (new, re-keyed) | — | #272 unit 2 emitter, after unit 4's re-keying | **Still parked (srs#526).** Sequenced **after** unit 4 (semanticObjectType collapse, #524) re-keys `requireSameSemanticObjectType` onto `requireSameType` — that unit has not landed. | #254 (left open — see #254's own row below, term.json), #372 |
...
| `manifest.json` | `Manifest` (new) | — | #272 unit 2 emitter | **Parked (srs#526).** Still coupled to unit 3: `renderedPresentations[].viewId` → `compositionId` cannot be modelled correctly until the Composition rename (#523) lands. | — |
| `view.json` | `View`/`FieldView`/`RecordPropertyView`/composite dispatch (new) | ext:views-l1 | #272 unit 2 emitter, sequenced after units 3 & 5 | **Parked (srs#526).** Still sequenced after the `exportConfig` relocation (unit 5, #525) and the Composition rename (unit 3, #523) — both open. RFC-041's `RecordPropertyView` row (merged, current name) is already live in this file today — not renamed, per the ruling not to pre-empt Composition. | #247 (left open, parked) |
...
| `lifecycle.json` | `Lifecycle` (new, installable/`lifecycleRef`-targeted — distinct from the inline `type-lifecycle` facet already in the metamodel) | ext:lifecycle | `rfc-272-closure-test.mjs` | **Generated (srs#526, Task 4b/2).** `states`/`transitions`/`initialState` reuse the RFC-040 `lifecycle-state`/`lifecycle-transition`/`initial_state` Fields verbatim — identical committed shape. One documented divergence: `states` (see projection-rules.md "Instance-layer entities") surfaces a pre-existing drift between `type.json`'s `TypeLifecycle.states` (has `minItems:1`) and this file's `states` (does not), and between the two files' `RequiresRelation.relationType` shape (list-only vs a stale pre-RFC-032-Rev-7 `oneOf`) — flagged as a Door-1 fixup candidate, not actioned here. | — |
...
conflict (`#390` / `package-bundle.json`), one new anomaly flagged for awareness (75 live orphan
`.revisions.json` files), and one new enumeration gap flagged for awareness
(`srs/package/records/`, 3 unvalidated Records) — none of which block srs#526's own completion.

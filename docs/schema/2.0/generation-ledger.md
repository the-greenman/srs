...
| `composition.json` (renamed from `document-view.json`, srs#523, this PR) | `Composition` (renamed, new) | ext:views-l2 | #272 unit 2 emitter | **Rename executed** — `$id`/`$schema` const/prose renamed, `SectionSource.type-query.semanticObjectType` → `typeKey` (KEYED, srs#524). Type-modeled generation still parked (srs#526). | — |

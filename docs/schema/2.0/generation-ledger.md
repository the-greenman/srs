...
| `theme.json` | `Theme` (new) | ext:themes-l1 | `rfc-534-closure-test.mjs` | **Generated (srs#534).** `theme` modelled with its full nested value-object subtree (`AssetDeclaration`, `PageTemplates`, `ElementTemplates`, `SectionWrapperOverride`, `RecordWrapperOverride`, `StylesheetDeclaration`, `TypographyHints`); `Theme.targets`' `minItems:1` uses the existing generic list-cardinality mechanism (`projectField` already threads `minItems`/`maxItems` for any list — no fix needed there, contrary to the unit's own working assumption). Excluded: `lineage`/`provenance` (bare `{type:object}` in this file, no real structure — same reasoning as Blueprint/Composition/View elsewhere in this ledger); `ElementTemplates.compositeRendererConfig` (deliberately unmodelled, not just parked — see below). **`Theme.assets` (map-of-`$ref` to `AssetDeclaration`) stays excluded too, but for a DIFFERENT reason — tooling parity, not modelling.** The map-of-`$ref` capability IS implemented and proven (`tests/rfc-534-emitter-capabilities/run.mjs`, synthetic fixtures), but populating the live `com.semanticops.srs/metamodel` corpus with it breaks `srs repo validate --repo srs` under the pinned srs-rust binary (build.320), whose embedded `field.json` schema predates the capability — same class of gap srs-rust#868 already parked a schema-touching change for; follow-up filed at srs-rust#932. `TableRendererConfig` (only ever nested inside the excluded `compositeRendererConfig`) needs no Type of its own either. **Generated reference (srs#534):** a `com.semanticops.spec/generated-type-reference` record now covers `Theme`. | #526 (theme.json row closed) |
| `discovery.json` | `DiscoveryQuery`/`TextSegment`/`ConformanceScenario`/`ExpectedSegments` (new) | ext:discovery | `rfc-534-closure-test.mjs` (via the new `emitDefsOnlyBundle` capability) | **Generated in full, including the standalone FILE itself (srs#534).** `TextSegment`, `ExpectedSegments`, and `ConformanceScenario` (three of the file's four `$defs` entries) are newly modelled, all with **zero exclusions**. The FILE's own standalone generation — previously parked because `discovery.json` has no top-level entity of its own, only a `$defs` bag — is now proven via `emitDefsOnlyBundle` (srs#534's second capability): it emits a `{$schema,$id,title,description,$defs}` bundle with no top-level object body, one `$defs` entry per named Type, keyed by PascalCase spelling matching the seed's own ad hoc convention. `rfc-534-closure-test.mjs` exercises the bundle directly (not merely defines it) and additionally asserts its per-entity output is self-consistent with `emitEntity`'s own per-type output. **`DiscoveryQuery.tier` stays excluded — tooling parity, not modelling (same reason as `Theme.assets` above, srs-rust#932).** The untyped-integer-enum capability IS implemented and proven (`tests/rfc-534-emitter-capabilities/run.mjs`), but adding `tier` to the live corpus's `discovery-query` Type breaks `srs repo validate --repo srs` under the pinned binary the same way `Theme.assets` would — so `DiscoveryQuery` stays exactly as modelled at srs#541 (`tier` excluded), unaffected in scope by this unit's own closure test still covering it cleanly. **Generated reference:** three new records now cover `TextSegment`, `ExpectedSegments`, and `ConformanceScenario`; `DiscoveryQuery`'s existing record is unchanged (still excludes `tier`, per srs#541). | — |
...
| **Generated (srs#534, 3 rows: theme + discovery.json's 4 entities/file + srsj-envelope); generated reference added same unit** | **3** | `theme.json` (`Theme` + 7 nested value objects; `Theme.assets` stays excluded — tooling parity, srs-rust#932, not a modelling gap), `discovery.json` (`TextSegment`/`ExpectedSegments`/`ConformanceScenario` newly modelled, plus the standalone FILE itself via the new `emitDefsOnlyBundle` capability; `DiscoveryQuery.tier` stays excluded, same tooling-parity reason as `Theme.assets`), and `srsj-envelope.json` (new file, `SrsjEnvelope` — the previously-owed `{srsj,manifest,data}` envelope, not one of the original 17 generatable rows but closed out this unit alongside them) |
| Generatable, still parked — a blocking issue, not an emitter-capability gap anymore | 2 | document-view-output (unit 3/#523, tracks #365), protocol* (blocked on #379) |
...
manifest, view, composition; 3 more at srs#534 — theme, discovery.json, plus the file-level bundle
proof — each with its own generated reference, srs#534 extending the srs#527/#274/#541 pattern).
Of the last 2 rows: one blocking prerequisite decision (`#379`, protocol.json); `document-view-
output.json` tracks its own three-way shape disagreement at #365 (unaffected by this unit). All
three of #534's own emitter-capability gaps (map-of-`$ref`, `$defs-only` bundle emission, untyped
integer enums) are now closed and proven (`tests/rfc-534-emitter-capabilities/run.mjs`, synthetic
fixtures for the two binary-parity-gated ones) — theme.json and the standalone discovery.json file,
the two rows they gated, are both generated. Two properties (`Theme.assets`, `DiscoveryQuery.tier`)
stay excluded from the LIVE corpus for a *fourth*, distinct reason found during this unit —
**tooling parity, not modelling**: the pinned srs-rust binary's embedded `field.json` schema
predates both new capabilities, so populating either Field in the self-hosted metamodel package
breaks `srs repo validate --repo srs` (same class of gap as srs-rust#868/#867); filed as
srs-rust#932, same disposition as the `FieldAssignment.description` precedent (land the mechanism,
park the corpus population). This does not narrow either row's "generated" classification any
more than `View.fieldViews`/`SectionSource`'s v2.1 exclusions narrow theirs. `#543` (discriminated-union / `oneOf` shapes: `Composition`'s
`SectionSource` and `View`'s `fieldViews`, found at srs#541) is disposed by the owner (2026-09-04):
**named v2.1 exclusion rows**, not blocking — see each entity's own row in §1 and the verbatim
disposition quoted there. `#390` (`package-bundle.json`) is disposed by the owner (2026-09-04):
**phase-scheduled** into its own dedicated roadmap phase, citing the ratified travel mandate
verbatim ("what a container can hold, a bundle can carry") — not part of the #256 closure gate,
not counted against the 17 generatable rows (same as before this unit). **Zero rows remain
unclassified or unnamed**: every row in this ledger is now one of generated / permanent-exclusion /
named-v2.1-exclusion (`#543`) / phase-scheduled (`#390`) / blocked-on-a-named-issue (`#379`, tracks
`#365`) / historical-retired / enumeration-classified. srs#254 closes in full (both its rows,
`term.json` and `relation-type.json`, are generated). One anomaly flagged for awareness and since
**resolved** (72 orphan `.revisions.json` sidecars deleted — srs#531, see §2's row above), and one
enumeration gap flagged for awareness and since **resolved** (`srs/package/records/` relocated into
`records/tier-2/` — srs#530, see §3's row above) — none of which blocked any unit's own completion.

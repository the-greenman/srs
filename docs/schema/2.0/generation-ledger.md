...
| `record.json` | `Record` (new) | — | `rfc-272-closure-test.mjs` | **Generated (srs#526, Task 4b/2).** `Record` modelled in `com.semanticops.srs/metamodel`; `fieldValues` uses the `dependent` datatype (approximated, deliberately lossy — see metamodel-fidelity.md); `fieldMeta` (a map-of-`$ref`) is excluded (no map-of-ref primitive exists without touching the frozen `field.json`). Authorship does not flip — `record.json` stays hand-authored/loaded as committed. | #248 (closed); #249 closed as obsolete (superseded by #248 — TypedRecord no longer exists post-#505) |
| `note.json` | `Note` (new) | — | `rfc-272-closure-test.mjs` | **Generated (srs#526, Task 4b/2).** `Note`/`NoteSection` modelled; `SourceReference` shared with Record/Relation. | #250 (closed) |
| `relation.json` | `Relation` (new) | — | `rfc-272-closure-test.mjs` | **Generated (srs#526, Task 4b/2).** Standalone since RFC-038. `$schema` const-pin not modelled (excluded, generic across every entity — no per-entity string-const `fieldType` primitive exists yet). | #251 (closed) |
...
| `relation-type.json` | `RelationTypeDefinition` (new, re-keyed) | — | #272 unit 2 emitter | **Re-keying landed (srs#523/#524, this PR); Type-modeled generation still parked (srs#526).** `requireSameSemanticObjectType`/`allowedSourceTypes`/`allowedTargetTypes` removed; `requireSameType` added (matches srs-rust#910's mirror byte-for-byte). Unit 2's metamodel-Type-generation of this schema is unblocked but not yet done. | #254 (left open — see #254's own row below, term.json), #372 (closed by the re-keying) |
| `container.json` | `Container` (new) | — | `rfc-272-closure-test.mjs` | **Generated (srs#526, Task 4b/2).** `containerType` excluded (already `deprecated: true` in the seed; no per-property deprecation mechanism modelled). | #252 (closed) |
| `manifest.json` | `Manifest` (new) | — | #272 unit 2 emitter | **`renderedPresentations[].viewId` → `compositionId` landed (srs#523, this PR); Type-modeled generation still parked (srs#526).** | — |
| `view.json` | `View`/`FieldView`/`RecordPropertyView`/composite dispatch (new) | ext:views-l1 | #272 unit 2 emitter, sequenced after unit 5 | **Composition rename landed (srs#523, this PR); semanticObjectType hint text now Type-key (srs#524, this PR); Type-modeled generation still parked (srs#526)**. `exportConfig` relocation (unit 5, #525) resolved: `View.exportConfig`/`ExportConfig` shape unchanged (owner ruling — one shape, two attachment points, no override), now also attached at `Composition.exportConfig`. RFC-041's `RecordPropertyView` row (merged, current name) is already live in this file today. | #247 (left open, parked) |
| `composition.json` (renamed from `document-view.json`, srs#523, this PR) | `Composition` (renamed, new) | ext:views-l2 | #272 unit 2 emitter | **Rename executed** — `$id`/`$schema` const/prose renamed, `SectionSource.type-query.semanticObjectType` → `typeKey` (KEYED, srs#524), then `type-query`/`typeKey` retired entirely in favor of `discovery-query`/`query: DiscoveryQuery` (srs#525, unit 5). `Composition` gains `exportConfig` (shared `ExportConfig` shape, duplicated locally per this file's existing convention), retiring its own inline `format`/`preamble` (srs#525, owner ruling — no more L1/L2 override semantics; `dataModelRevision: 7`). Type-modeled generation still parked (srs#526). | — |
...
| `source-document-meta.json` | `SourceDocumentMeta` (new) | — | `rfc-272-closure-test.mjs` | **Generated (srs#526, Task 4b/2).** `SourceExcerpt`/`SourceAnchor` value objects modelled; `imported_at` reused from the frozen bootstrap's `Provenance.importedAt` Field (same semantic meaning). | — |
...
| `discovery.json` | `DiscoveryQuery` (new) | ext:discovery | #272 unit 2 emitter | **Parked (srs#526).** Two genuine emitter-capability gaps, not oversights: (1) `discovery.json` has no top-level entity — it is ALL `$defs` (`DiscoveryQuery`/`TextSegment`/`ConformanceScenario`/`ExpectedSegments`, four co-equal shapes), but `emitEntity` only knows how to emit a single top-level object; a new "`$defs`-only bundle" emission mode would be needed. (2) `DiscoveryQuery.tier` is a bare, untyped integer enum (`{"enum":[0,2]}`, no `type` key) — the closed-enum mechanism today only supports string domains. Recommend a small follow-up unit extending the emitter for both, rather than forcing a mismatched model onto the frozen shape now. Still load-bearing for unit 5's SectionSource → DiscoveryQuery collapse. | — |
...
|---|---:|---|
| **Generated (srs#526, Task 4b/2)** | **9** | record, note, relation, container, blueprint, term, vocabulary, lifecycle, source-document-meta |
| Generatable, still parked on a later unit or a blocking issue | 7 | relation-type (unit 4/#524), manifest (unit 3/#523), view (units 3&5/#523,#525), document-view→composition (unit 3/#523), document-view-output (unit 3/#523), protocol* (blocked on #379), theme (needs a follow-up unit — new nested Types + map-of-ref + an intentionally-unconstrained bag), discovery (needs a follow-up unit — new "$defs-only" emitter mode + untyped-integer-enum support) |
...
**22 live schema files, all classified. Zero unclassified schema files. 9 of the 16 "generatable"
rows are now generated** (srs#526, Task 4b/2 — Task 4b/1's original count of 16, above). The
remaining 7 are named individually with the specific reason each is still parked, not a residual
"not done yet." One blocking prerequisite decision (`#379`, protocol.json), one open mechanism
conflict (`#390` / `package-bundle.json`), one anomaly flagged for awareness and since **resolved**
(72 orphan `.revisions.json` sidecars deleted — srs#531, see §2's row above), and one enumeration
gap flagged for awareness and since **resolved** (`srs/package/records/` relocated into
`records/tier-2/` — srs#530, see §3's row above) — none of which blocked either unit's own
completion, since #522's job was the ledger and the owner-review gate and #526's was the nine
generated entities, not resolving these.

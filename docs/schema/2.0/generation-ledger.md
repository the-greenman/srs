...
| `relation-type.json` | `RelationTypeDefinition` (new, re-keyed) | — | #272 unit 2 emitter | **Re-keying landed (srs#523/#524, this PR); Type-modeled generation still parked, verified not merely assumed (srs#527's ledger walk); no `com.semanticops.srs/metamodel` Type exists for this entity today.** `requireSameSemanticObjectType`/`allowedSourceTypes`/`allowedTargetTypes` removed; `requireSameType` added (matches srs-rust#910's mirror byte-for-byte). The metamodel-Type-modeling this schema needs is filed as srs#541. | #254 (left open — see #254's own row below, term.json), #372 (closed by the re-keying) |
...
|---|---:|---|
| **Generated (srs#526, Task 4b/2); generated reference added (srs#527, Task 4b/6)** | **9** | record, note, relation, container, blueprint, term, vocabulary, lifecycle, source-document-meta — plus 3 nested value objects (note-section, source-reference, relation-spec) also given their own generated reference |
| Generatable, still parked — metamodel Type-modeling filed as its own follow-up (srs#541) or a blocking issue | 7 | relation-type (srs#541), manifest (srs#541), view (srs#541), document-view→composition (srs#541), document-view-output (unit 3/#523), protocol* (blocked on #379), theme (needs a follow-up unit — new nested Types + map-of-ref + an intentionally-unconstrained bag), discovery (needs a follow-up unit — new "$defs-only" emitter mode + untyped-integer-enum support; the nested copy inside composition.json's SectionSource is in scope for srs#541 modulo the same `tier` exclusion) |
...
Zero unclassified schema files. 9 of the 16 "generatable" rows are generated, and all 9 now also
carry a generated reference** (srs#526, Task 4b/2 for the schema-closure proof; srs#527, Task 4b/6
for the reader-facing property table — Task 4b/1's original count of 16, above). The remaining 7 are
named individually with the specific reason each is still parked: four (relation-type, manifest,
view, composition) have their metamodel-Type-modeling filed as srs#541 (not merely "later", now
tracked); one blocking prerequisite decision (`#379`, protocol.json); one open mechanism conflict
(`#390` / `package-bundle.json`); theme and discovery stay gated on the emitter capability gaps
tracked separately (map-of-$ref, $defs-only bundling, untyped integer enums) — not this train's to
resolve. One anomaly flagged for awareness and since **resolved** (72 orphan `.revisions.json`
sidecars deleted — srs#531, see §2's row above), and one enumeration gap flagged for awareness and
since **resolved** (`srs/package/records/` relocated into `records/tier-2/` — srs#530, see §3's row
above) — none of which blocked any unit's own completion.

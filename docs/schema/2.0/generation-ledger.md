...
generated. Two properties (`Theme.assets`, `DiscoveryQuery.tier`) stay excluded from the LIVE corpus
for a distinct reason found during this unit — **tooling parity, not modelling**: the pinned
srs-rust binary's embedded `field.json` schema predates both new capabilities, so populating either
Field in the self-hosted metamodel package breaks `srs repo validate --repo srs` (same class of gap
as srs-rust#868/#867); filed as srs-rust#932, same disposition as the `FieldAssignment.description`
precedent (land the mechanism, park the corpus population). This does not narrow either row's
"generated" classification any more than `View.fieldViews`/`SectionSource`'s v2.1 exclusions narrow
theirs. `#543` (discriminated-union / `oneOf` shapes: `Composition`'s `SectionSource` and `View`'s

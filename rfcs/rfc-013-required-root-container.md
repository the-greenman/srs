...
| 7 | 2026-08-28 | **Amended by srs#446** (pattern-consistency audit, Membership signature, `rfc-decision-cce3c00e`). **[R9] tightened**: the prior text tolerated a Container elsewhere in the container set declaring the same `containerId` as `manifest.container`, resolved by precedence (treat `manifest.container` as authoritative regardless). RFC-038 [R25] made the container set the authority R9 resolves against, and RFC-038 [R12] makes a duplicate logical identifier in the same authoritative set **fatal, with no precedence resolution** — the two rules now conflict on this exact case. [R12] wins (Earth: identity conflicts are fatal, never resolved by precedence); R9's precedence-tolerance clause is withdrawn. **[R5] confirmed, no textual change**: R5's "unordered set, MUST NOT derive order from position" premise previously held only for navigation order, while RFC-009 Rev ≤5 relied on `rootInstanceIds[0]` for the typing anchor — a second, contradictory positional use the pattern-consistency audit flagged as finding B3. RFC-009 Rev 6 (srs#446) replaces that positional reliance with the explicit `Container.anchorInstanceId` field, so R5's unordered-set premise now holds without exception, for typing as well as navigation. |
...
>
> **[R9]** *(Amended by RFC-038 [R25] — resolves against the container set, not `containerIndex`; further amended by srs#446.)* The root container is identified solely by `manifest.container`; it MUST NOT be required to appear elsewhere in the repository's container set. A Container entry elsewhere in the container set declaring the same `containerId` as `manifest.container` is a **duplicate logical identifier in the same authoritative set** — this is FATAL under RFC-038 [R12] (no precedence resolution). An implementation MUST NOT silently treat `manifest.container` as authoritative over such a duplicate; the repository is invalid until reconciled to a single root-container representation (as the `muSrs` migration in RFC-038 required in practice).
>

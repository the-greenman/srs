...
**Status**: Superseded (by RFC-033)

> **Supersession note (RFC-033, #266)**: RFC-004's separate schema-notation vocabulary — `ext:schema-notation`, the `com.semanticops.spec/schema-definition` and `schema-member` Types, and the structured `typeExpression` kinds — is **superseded** by the self-hosted `com.semanticops.srs` Field/Type meta-model (RFC-033). There is now one schema-source vocabulary: the Field/Type meta-model itself. RFC-004's `projection-rules.md` (`rfcs/rfc-004/proposed-package/spec-authoring-json-schema/projection-rules.md`) is **retained and reused** as the normative source → JSON Schema projection contract for the #259 emitter. The never-loaded proposed artifacts below remain historical design material and MUST NOT be loaded into an active `package/`.

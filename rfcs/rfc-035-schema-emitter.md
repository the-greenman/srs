...
| Rev | Date | Summary |
|---|---|---|
| 6 | 2026-08-25 | **Amendment (RFC-040 Unit 3, srs#479), not a re-open:** the "CLI + WASM parity: Deferred to #260" Testability row below is stale — the `srs-projection` Rust twin (`json_schema.rs` + `rfc_035_parity.rs`) has since shipped; the #260 deferral was always about the **authorship flip** (whether the seed or the emitter owns the committed bytes), never about whether a Rust emitter exists. Parity between the Node reference emitter and the Rust twin is now a standing closure criterion, tracked via the srs-rust mirror-sync follow-up filed at each schema-touching unit's landing. See `docs/schema/2.0/projection-rules.md`'s own amendment note for the fuller byte-closure-contract update this same unit made. |
...
| CLI + WASM parity | **Stale as of Rev 6 (RFC-040 Unit 3, srs#479) — the `srs-projection` Rust twin has shipped**, not "deferred to #260": #260 is the seed/emitter **authorship flip**, a separate question from whether a Rust emitter exists. The twin (`srs-projection/json_schema.rs` + `rfc_035_parity.rs`) implements to the pinned `projection-rules.md` contract; parity with the reference goldens is a standing closure criterion, verified via the srs-rust mirror-sync follow-up filed at each schema-touching unit's landing (not built or verified in this repo — ADR-004). |

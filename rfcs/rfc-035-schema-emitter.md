...
| Rev | Date | Summary |
|---|---|---|
| 7 | 2026-08-26 | **Amendment, not a re-open:** cited the shipping PR (srs-rust#778 — "adopt the RFC-032 fieldType model + the RFC-035 JSON Schema projection (#767–#770)", merged 2026-07-31) alongside Rev 6's "has since shipped" claim and the Testability row's matching claim below, and alongside `projection-rules.md`'s own parity-status paragraph — naming the PR rather than leaving the shipped state uncited. Also annotated the Abstract's own still-unamended "specified here and implemented at the #260 cutover" sentence (srs#428's third named location) as stale, pointing to the Testability row and this revision history rather than rewriting the original scope decision. No change to the underlying claims. Refs srs#428. |
| 6 | 2026-08-25 | **Amendment (RFC-040 Unit 3, srs#479), not a re-open:** the "CLI + WASM parity: Deferred to #260" Testability row below is stale — the `srs-projection` Rust twin (`json_schema.rs` + `rfc_035_parity.rs`) has since shipped (srs-rust#778) for RFC-035's original scope; the #260 deferral was always about the **authorship flip** (whether the seed or the emitter owns the committed bytes), never about whether a Rust emitter exists. It does **not yet** implement RFC-040's own additions (effective-Type resolution, the facing distinction, conditional projection) — verified directly against `crates/srs-projection/src/json_schema.rs`, which has no reference to `extends_type_id`/`field_order`/`field_assignment_overrides`/conditional `allOf` projection. That gap is named precisely in the srs-rust follow-up filed at this unit's landing (a comment on srs-rust#868). Parity remains the eventual closure criterion; it is not yet held for RFC-040's additions specifically. See `docs/schema/2.0/projection-rules.md`'s own amendment note for the fuller byte-closure-contract update this same unit made. |
...
| 5 | 2026-07-29 | **Accepted; reference emitter authored, Node pipeline green.** Shipped `scripts/lib/schema-emitter.mjs` (neutral IR + JSON emitter reusing `projectField`), the Tier-1 determinism goldens (`tests/rfc-035/`, byte-for-byte over `field.json`/`type.json` + the stamped bundle envelope), and the Tier-2 `emitter ⊆ seed` closure (`scripts/rfc-035-closure-test.mjs`: 20 authoritative properties consistent, 1 registered `type.aiGuidance` divergence, exclusion set printed). Relocated `projection-rules.md` → `docs/schema/2.0/` (re-anchored to Field/Type + ordering/override/register/bundle/amendment additions); updated the fidelity-dashboard verification note; wired both tiers into `scripts/validate-all.mjs`. Authored the RFC-035 stub record + `tooling-only` integration manifest. |
| 4 | 2026-07-29 | Implementation started; RFC committed to branch `rfc/035-schema-emitter`. Building the Node reference emitter, the Tier-1 determinism goldens, the Tier-2 closure, the `projection-rules.md` relocation, and the fidelity-note update. |
...
load the `fieldType` package (ADR-004) — exactly as RFC-033 deferred its own embedded-bundle wiring. (**Stale
as of Rev 6/7, RFC-040 Unit 3 / srs#479, srs#428:** the `srs-projection` core's JSON Schema projection —
`type_to_json_schema` — has since shipped, srs-rust#778, for RFC-035's original scope; #260 remains the
seed/emitter **authorship flip**, not the existence of a Rust emitter. See the Testability table's "CLI +
WASM parity" row and the revision history above.)
...
| CLI + WASM parity | **Stale as of Rev 6 (RFC-040 Unit 3, srs#479) — the `srs-projection` Rust twin has shipped (srs-rust#778) for RFC-035's original scope**, not "deferred to #260": #260 is the seed/emitter **authorship flip**, a separate question from whether a Rust emitter exists. The twin (`srs-projection/json_schema.rs` + `rfc_035_parity.rs`) implements the pinned `projection-rules.md` contract as RFC-035 originally specified it; it does **not yet** implement RFC-040's Change A/F/G additions (effective-Type resolution, facing, conditional projection) — that gap is named precisely in the srs-rust follow-up filed at this unit's landing (a comment on srs-rust#868), not built or verified in this repo (ADR-004). Parity with the reference goldens over RFC-035's original scope still holds; parity over RFC-040's additions is the follow-up's job, not yet closed. |

...
| 4 | 2026-07-02 | Address Revision 3 review findings: add R10 manifest-only-validator exemption; add R10 rationale; label muDemocracy.org fixture reference informational. Zero blocking findings — RFC accepted. |
| 5 | 2026-07-07 | Accepted; spec records authored in srs/srs (ext:import-tracking extension record, Invariants 83–84); schema changes applied to docs/schema/2.0/manifest.json. |
...
**Conformance fixture:** a conformance fixture demonstrating a multi-version install MUST be authored under `conformance/import-tracking/` and committed with the schema changes as part of RFC acceptance. The fixture must include at minimum: (a) a `manifest.json` with two `PackageRef` entries for the same `packageId` at different `packageVersion` values, (b) at least one record file whose `typeId` resolves in the older package version directory but not the newer, and (c) a `README.md` summarizing what the fixture demonstrates. The muDemocracy.org governance upgrade (1.0.0 → 1.1.0, muDemocracy.org#55/#56) serves as the primary acceptance fixture (informational).
...
### Why does R10 exempt manifest-only validators when no `PackageRef` carries `packageId`?

Pre-RFC-014 local-mode `PackageRef` entries omit `packageId`. A validator that can only read `manifest.json` (and not the local package manifest files) cannot determine the `packageId` of those entries. Requiring such a validator to report an error would produce false positives for all pre-RFC-014 repositories — the wrong outcome. The exemption limits R10 enforcement to contexts where the information is actually available.

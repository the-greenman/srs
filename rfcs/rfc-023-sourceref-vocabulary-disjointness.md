...
- **RFC-029 (core base package & identity type)**: the migration posture follows its R7 grace-period precedent — warning during the window, enforcement after a spec-visible event — with one difference: RFC-029 R7's window is open-ended, while this RFC's window closes at a scheduled schema revision (tracked on #171).

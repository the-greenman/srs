...
| 8 | 2026-09-11 | **Erratum (post-acceptance), Door 3 — the-greenman/srs#729.** [R3] said `valueDomain` is meaningful only for `datatype == "string"`; the implementation (srs-rust, per srs#534) has always accepted `datatype == "integer"` too, and the spec repo's own metamodel package depends on the widened reading: `com.semanticops.srs/tier` (`srs/package/metamodel/fields/tier.json`) is `datatype: integer, valueDomain: closed, allowedValues: [0, 2]` — a violation under the RFC's literal text, legal under the implementation. Ruled: keep the widened reading (it is what the corpus, `scripts/lib/rfc-032-fieldtype.mjs`'s `validateFieldType`, and `scripts/lib/schema-emitter.mjs` already assume); [R3] is corrected to match. Scope stays narrow — `string` or `integer`, not every scalar datatype: no other datatype uses `valueDomain` anywhere in the corpus, and `vocabularyRef` (RFC-006 Vocabularies are string Terms) is unaffected — only the inline-`allowedValues` closed-domain path gains the integer case, matching `schema-emitter.mjs`'s untyped-integer-`enum` projection (srs#534) and `docs/schema/2.0/metamodel-fidelity.md`'s existing "Untyped integer enum" row. `docs/schema/2.0/field.json` was never schema-restricted to `string` here, so no schema edit is required — this closes prose-only drift. |
...
>
> **[R3]** `valueDomain` is meaningful only for `datatype == "string"` or `datatype == "integer"`
> (widened from string-only by srs#534, ratified Revision 8). When `closed`, exactly one of
> `allowedValues` or `vocabularyRef` MUST be present; a `vocabularyRef` MUST resolve to a `mode:closed`
> Vocabulary (RFC-006) — Vocabularies are string Terms (RFC-006), so `vocabularyRef` remains
> string-only even where `allowedValues` is not.
>

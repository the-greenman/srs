...
| Rev | Date | Summary |
|---|---|---|
| 11 | 2026-08-09 | srs#242 Phase B: Open Question 2's metamodel example-instance round-trip obligation (:407, assigned to #242) is explicitly re-booked to instance-layer self-hosting (#272/#273), where metamodel entities will exist as Tier-2 Records to round-trip — the owner-confirmed D3 disposition on srs#242. RFC-039 OQ1 records the same decision from its side. |
...
| 5 | 2026-07-31 | **Erratum (post-acceptance), from [#295](https://github.com/the-greenman/srs/issues/295). Ratified by the repository owner 2026-07-31** (the-greenman/srs#256) — flagged rather than carried silently, per the Rev 6 precedent on RFC-032. Change A's UUID scheme pinned the package id `4a000001-0000-4000-a000-000000000001` without checking the shared `4a……` package series for prior claims; `com.semanticops.spec/spec-authoring-core` already held it, and both packages are resolved by `srs/manifest.json`, so every consumer keying packages by id (a registry, RFC-014 import tracking, a bundle index, a cache) saw one package where there are two — resolution-order dependent. Two entities sharing a UUID contradicts the identity rule the whole model rests on. The metamodel package is reassigned to **`4a000004-…-004`**, the first slot no tree in this repo has ever claimed (`…-002` = `spec-rfc-process`, `…-003` = RFC-004's proposed `spec-authoring-json-schema` — superseded and never installed, but a claimed id is not recycled). Reassigning the *generated* side is the cheaper correction: `scripts/gen-metamodel-package.mjs` holds the pin and regenerates its 66 files, where `spec-authoring-core` is hand-authored with 13 Types and 28 Fields hanging off it. Field (`4b……`) and Type (`4c……`) assignments are untouched and were never in conflict. `scripts/check-package-id-uniqueness.mjs` (wired into `validate-all.mjs`) now fails any duplicate resolved package or definition id, and any package id naming two different `namespace/name` pairs across the repo's package trees — nothing detected this class of defect before. Metamodel package id only; no normative change. |
| 4 | 2026-07-29 | **Accepted; spec artifacts authored.** Generated the `com.semanticops.srs/metamodel` frozen-seed package (10 Types + 55 Fields, `scripts/gen-metamodel-package.mjs` with `--check`) — validates against the frozen seed (0 errors). Bootstrap-closure harness (`scripts/rfc-033-closure-test.mjs`): 50 authoritative matches + 12 documented-approximated, demonstrating [R4](a). Committed `docs/schema/2.0/metamodel-fidelity.md`, the LinkML alignment entry, the three additive `dataModelRevision` schema fields + frozen-seed `$comment`s, the `dataModelRevision: 1` re-stamp of `srs/manifest.json`, RFC-004 → Superseded (both `.md` status lines + stub record), and the RFC-033 stub record + `srs-integration:v1` manifest. All Node validations green (`scripts/validate-all.mjs`). |
...
| Version | `1.0.0` |
| UUID scheme | patterned, human-legible (mirrors RFC-029 core): `4a……` = package, `4b……` = fields, `4c……` = types. **Assignment is pinned by declaration order:** package `4a000004-0000-4000-a000-000000000004` (Rev 5 erratum — Rev 4 pinned `4a000001-…-001`, which `com.semanticops.spec/spec-authoring-core` already held; `…-002` and `…-003` were likewise taken, see [#295](https://github.com/the-greenman/srs/issues/295)); Types numbered `4c0000NN` in the "Self-hosted Types" table order below (`field` = `4c000001`, `type` = `4c000002`, …, `provenance` = `4c00000a`); Fields numbered `4b0000NN` in the order they first appear in the derivation (Change A, "Derivation rule"). Deterministic, not random v4. |

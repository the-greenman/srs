...
| Rev | Date | Summary |
|---|---|---|
| 13 | 2026-09-17 | **Reverses Revision 7's disposition for `conformance/discovery/fixture-repo`, owner ruling on srs-rust#1024 (2026-09-17, comment 5713404093): "keep fixtures up to date."** Revision 7 (2026-08-01) kept the discovery fixture as test data specifically to avoid "inventing repository identity" for it; the owner revisited that trade-off directly on the issue that found the resulting gap (the fixture is permanently unreachable by `repo validate`, so a definition-layer rule it violated would never be caught there) and chose migration over documenting the gap. The fixture now carries this RFC's full storage contract: `apply-migration --id rfc038-storage` strips `instanceIndex`/`containerIndex`/`relationsPath` and removes the (always-empty, 0-relation) `relations/relations.json` collection file; `repo set-root-container` declares `manifest.container` — `containerId 0000c003-0000-4000-8000-000000000003`, `identityInstanceId` the `SRS Discovery Contract` article (`00000001-…-000000000001`), all 9 instances in `memberInstanceIds` (the fixture has no `contains` relations, so there is no nested structure to express via `rootInstanceIds`/`childContainerIds`). **Two accepted, warning-level gaps, deliberately not closed**: (1) the identity instance resolves to `com.example.discovery/article`, not the `com.semanticops.core/purpose` Record RFC-029 I-87 requires on a root container — closing it would pull a `com.semanticops.core` package reference into an otherwise minimal, discovery-only test package, which this revision judges out of scope for a storage-shape migration; `repo validate` reports it as a **warning**, and neither `repo validate`'s own exit semantics nor `check-corpus-conformance.sh` (srs-rust#877, which only fails on `errors != 0` or a zero-instance load) treat a warning as blocking. (2) each of the other 8 members trips RFC-013 I-82 ("not the root of any container in the container set") — expected for a fixture with zero container nesting to test. **The `[R21]`-independent reader this fixture motivated is not retired**: `FileStore::with_rfc038_exemption` stays in `srs-rust`, used by unrelated pre-RFC-038 test fixtures (`migration_registry_service`'s `indexed_srsj_store`, `migration_ladder.rs`'s `rev2_fixture`, `srs-bindings/tests/migration_registry.rs`); only `discovery_conformance.rs`'s call on **this** fixture is dropped, in the matching `srs-rust` PR. That same PR adds this fixture to the release corpus-conformance gate (srs-rust#877) — the reach gap srs-rust#1024 exists to close. No `.srs` marker is added and the fixture package's Revision-10 stamping-inventory membership is unchanged — this revision is a storage-shape migration only, not a reclassification of the fixture into the first-party corpus-census population Revisions 3/8/10 track. No other rule text changes; this is a Door-3 record of an owner disposition, the same pattern Revision 9 used for Phase 0 step 2. |
| 12 | 2026-09-02 | **Retires [R3]'s package-root instance-anchor branch (owner ruling, 2026-09-02, verbatim): "lets retire this. at the moment packages should be semantic not content. We have other proposed ways to distribute content as slices."** Packages carry definitions — the semantic composition of Fields into Types (the definition set, [R5]); content distribution is the slices path (RFC-026) or seeds, never a nested `records/` under a local package root — one mechanism per goal. [R3] now reads: a reserved instance root is a `records`/`notes`/`typed-records` directory that is an immediate child of the **repository root only**; the "or of a directory containing a `package.json` that validates against `package-manifest.json`" clause, and the now-vacuous "a package root nested inside a reserved instance root MUST NOT anchor further roots" sentence it existed to bound, are both removed. **Descriptive origin, so the branch being retired is not mistaken for a considered architectural choice**: Revision 2 and Revision 3's closure discipline measured an independent implementation of this branch against the corpus and found it added exactly 7 instances at `srs/package/records/` — but those 7 existed only because RFC-011's own stub and change records were misfiled there, not because a package legitimately needed to carry instances. 3 of the 7 (RFC-011's accepted invariants) were relocated to the projection root by #410; the remaining 4 (RFC-038's own stub record and its three `rfc-change` records) were diagnosed as validated by nothing by #530 and relocated to `records/tier-2/` by PR #536, leaving the location class with zero occupants before this amendment retires it. `scripts/lib/rfc-038-tree.mjs`'s matching `instanceRoots()` branch is removed in the same PR that lands this revision, red-then-green against a planted `package/records/` fixture. No canonical spec record yet mirrors this branch of [R3] — RFC-038's full fold remains grandfathered under `rfcs/integration-allowlist.json` pending #297 — so this revision is the only site retiring it; the fold, when #297 lands, inherits the amended rule directly. **Return trigger**: a package needing to ship records rather than slices or seeds re-opens this with the use case. |
| 11 | 2026-08-13 | **Schema cutover lands (#297 unit 1) and the stamping-order deviation is recorded.** The Schema-changes table's four editable rows are implemented in `docs/schema/2.0/`: `manifest.json` drops the five retired properties, their four `$defs` and the `instanceIndex` `required` entry; `relation.json` is created (17 properties / 5 required, `additionalProperties: false`, `const`-pinned `$schema`); `relations-collection.json` is redescribed as a generation-≤1 artifact; `relation-type.json` gains `additionalProperties: false`. The deviation: the composition section pinned an ordering in which the corpus is *"never simultaneously stamped and un-stripped"*, and #242 Phase B shipped the stamp alone — the corpus sat at `dataModelRevision: 2` with `instanceIndex` present from 2026-08-10 until the srs-rust#783 Phase-6 enforcement flip, a transient [R2]-non-conformant state recorded on #297 rather than blessed with an exemption (owner disposition 2026-08-10: do not amend the rule; a rev-2-with-index repository remains an error). The durable lesson: the pinned ordering existed only in prose — nothing enforced it, so it did not hold. The enforcement flip closes that class: [R2]/[R21] are now reader-enforced, so the stamped-but-unstripped state can no longer exist silently. No rule text changes. |
| 10 | 2026-08-05 | Folds `conformance/discovery/fixture-repo/package/package.json` into the single stamping inventory's in-scope bucket (Package manifests, first-party in-scope: 11 → 12), closing an omission where the file fell into neither bucket. Per the owner's package/repository split (the-greenman/srs#256 body, *First-party corpus boundary — owner decision*, recorded on #314): fixture **packages** reach the contract even though the fixture **repository** stays test data and receives no `.srs` marker — `[R21]` binds "a repository **or** package" disjunctively, the same reading Revision 8 used to pull `core-bundle.srsj` into the same bucket. This reopens no decision and adds no policy; it corrects the inventory table the Revision 8 note itself exists to keep singular and complete. |
...
repositories there are **243 relations across 3 non-empty collections**. The relations-storage rule requires a collection object, so
...
Deriving kinds from the manifest's arrays also surfaces which of them have no schema to validate
against, and one does not: there is **no `protocol.json` under `docs/schema/2.0/`**, while both
governance packages declare a protocol that itself carries no `$schema`. [R8] therefore has nothing to
validate it against, [R9] makes it an error and [R24] makes that fatal. The schema is owed and is a
Phase-0 prerequisite; narrowing the admissible kinds instead would reintroduce exactly the hand-kept
list this paragraph rejects.
...
`relations/relations-collection.json` `data` key, and it is converted rather than preserved.
...
> directory containing both the `.srs` marker and `manifest.json`. Discovery MUST recurse through the
> entire subtree. The anchor search MUST be confined to the repository subtree and MUST skip `.git/`,
> `node_modules/` and `.srs/`. Reserved instance roots MUST NOT be determined from `packageRefs` or any
> other manifest-declared path list. A local package root (Change B) MUST NOT anchor an instance root:
> packages carry definitions (the definition set, [R5]), never instances — a package that needs to
> distribute content uses a slice (RFC-026) or a seed (Revision 12).
...
> over the encompassing package root. Every schema in an admissible set MUST be
> `additionalProperties: false`. Where an admissible set has **more than one** member its schemas MUST
> additionally remain pairwise distinguishable: each MUST have at least one `required` property that
> is not a declared property of any other schema in the set. A change that breaks either property MUST
> be rejected. The distinguishability conjunct is scoped to multi-member sets deliberately: a declared
> definition path resolves to exactly one kind, so its admissible set is a singleton and has nothing to
> discriminate against. Both conjuncts nonetheless require one schema change — `relation-type.json`
> does not set `additionalProperties` today, and RelationType is a declared definition kind (23 in
> `srs/`, 5 in each governance package), so the blanket clause is false until it does.
...
| `relation.json` | **New file.** Standalone single-Relation entity: the 16 inherited properties of Change E plus the required `const`-pinned `$schema`, i.e. **17 properties / 5 required**, `additionalProperties: false` | **243 relations across 3 non-empty live collections** become one file each: 205 (`srs/`), 17 (`gallery-project-v2/`), and 21 (`muSrs/`) |
| `relations-collection.json` | Retained only to describe generation-≤1 artifacts; **not** a generation-2 format in live repositories *or* snapshots | The 3 non-empty live collection files are removed after conversion; `gallery.srsj`'s embedded collection is converted too. `conformance/discovery/fixture-repo/relations/relations.json` is a 4th collection file but holds **0 relations** and is fixture data, not a migrated repository |
| `relation-type.json` | Add `additionalProperties: false` | No conforming RelationType changes; the edit is what makes [R8]'s blanket clause true. 23 RelationTypes in `srs/` and 5 in each governance package are already closed in practice |
...
| *`protocol.json`* | **New file, owed.** `package-manifest.json` declares a `protocols` array, but **no protocol definition schema exists anywhere under `docs/schema/`** | `packages/com.mudemocracy.governance/{1.0.0,1.1.0}/package/protocols/decision-7a088176.json` declares no `$schema`, so [R8] must validate it against the declared definition entity and cannot. [R9] makes it an error and [R24] makes that fatal, so **both governance packages fail to load** at the generation this cutover stamps them to. Phase-0 prerequisite |
...
3. Resolve srs#307 (Change H). It is fixed on its own row, but it **must** be fixed before the
   ...
   Two schema gaps sit in this same fatal-prerequisite bucket and are resolved here rather than
   registered. **Land the owed `protocol.json` definition schema.**
   `package-manifest.json` declares a `protocols` array; both
   `packages/com.mudemocracy.governance/{1.0.0,1.1.0}/package/package.json` declare
   `protocols/decision-7a088176.json`; that file declares no `$schema`, so [R8] must validate it
   against the declared definition entity and there is none to validate against. Revision 7 brings
   both governance packages into this cutover, so without the schema they are stamped to a generation
   at which they cannot be loaded. The alternative — removing `protocols` from the admissible
   definition kinds — is **not** taken, because Change A derives kinds from the package manifest's own
   arrays precisely so a declarable kind cannot be unrepresentable. Add `additionalProperties: false`
   to `relation-type.json` in the same step, which is what makes [R8]'s blanket clause true.
...
5. Record baseline counts, identifier sets and content hashes for parity checking: **417 Tier-2, 2
   ...
   3 non-empty collections** — 205 (`srs/`), 17 (`gallery-project-v2/`), 21 (`muSrs/`) — measured per
   tree. A 4th collection file exists at `conformance/discovery/fixture-repo/relations/relations.json`
   and holds **0 relations**; the fixture is not a repository under the dispositions below, so it is
   neither counted nor migrated. Preserve both sides of every explicitly reconciled conflict in
...
7. Write each relation in the three non-empty collections to `relations/<relationId>.json`; verify the
   `relationId` set is unchanged and unique; delete those collection files. This includes the 21
   relations in `muSrs/relations/relations-collection.json`. The fixture collection is left alone.
8. **The single shared stamping step** (see the ordering above): strip the retired inventories from
   ...
   | Package manifests, first-party in-scope | 12 | 6 under `srs/package/` + gallery + `muSrs/package/` + `muSrs/packages/governance/` + `packages/com.mudemocracy.governance/{1.0.0,1.1.0}/package/` + `conformance/discovery/fixture-repo/package/` — the fixture *package* reaches the contract even though its enclosing fixture *repository* stays test data ([R21] binds "a repository **or** package") |
   ...
   | Package bundles taking `dataModelRevision: 2` only | 1 | `core-bundle.srsj` — no `srsj`, no `manifest`, no `data`, so [R19]/[R20] do not reach it. But [R21] binds "a repository **or** package", and an unstamped bundle is generation 0, which every conforming reader must then reject. `package-bundle.json` already declares `dataModelRevision`, so the fix is the stamp, not a schema change |
...
16. Both `com.mudemocracy.governance` packages load at generation 2 with their declared protocol
    classified against the new `protocol.json`, and every schema in every multi-member admissible set
    still satisfies [R8]'s two conjuncts — including `relation-type.json`, which acquires
    `additionalProperties: false` in this cutover.
17. `core-bundle.srsj` carries `dataModelRevision: 2` after the cutover and is accepted by a
    generation-2 reader; the same bundle unstamped is rejected under [R21].
...
| `docs/spec/examples/gallery-project-v2/` | own | added by this PR | 2 T0, 22 T2 | 2 / 2 T0, **0 / 22 T2** |
...
**Relations: 243 across 3 non-empty live collections** — 205 in `srs/` (99 `precedes`, 106
`contains`), 17 in `gallery-project-v2/`, and 21 in `muSrs`; all 21 `muSrs` relation endpoints resolve
against its 32-instance set. A 4th collection file, `conformance/discovery/fixture-repo/relations/relations.json`,
holds 0 relations and belongs to a fixture rather than a repository, so it is outside the migration.
...
invisible to every manifest-based enumeration including the current one — and it stays invisible,
because the dispositions below keep it as unit-test input rather than giving it repository identity.
It would become reachable under this RFC's rules only if it acquired a manifest, which this migration
does not do.
...
files are an elimination. The cost is 243 small files where there were 3, which is what the filesystem
...
- The owed `protocol.json` and the `relation-type.json` closure are the same class: implementation
  tasks that the fail-closed posture makes prerequisites rather than follow-ups. Neither adds a policy
  choice, and neither has an alternative that does not narrow the definition-kind set Change A
  deliberately derives from the package manifest.
- `core-bundle.srsj` is a published first-party package and is stamped with its consumers. Leaving it
  unstamped is not a deferral option, because [R21] makes generation-0 data unreadable rather than
  merely stale.

...
**Affects**: `ext:views-l2` (`Composition.exportConfig`, formerly `DocumentView.format`/`.preamble`; *Revision 11*: Rule [N+1]'s per-record heading clause and the Heading Hierarchy table), `ext:views-l1` (`ExportConfig`)  
...
| 10 | 2026-09-03 | ExportConfig unification (srs#525, owner ruling): `DocumentView`/`Composition`'s own inline `format`/`preamble` retire in favor of `Composition.exportConfig` — the same shared `ExportConfig` shape this RFC's Rev 4/5 already gave `View` (`ext:views-l1`), now attached at two points instead of one. **No precedence/override semantics between the two attachments** (this revision retires Rev 4/5's "DocumentView.format vs ExportConfig.format... supersedes" framing): `Composition.exportConfig` governs document-level rendering (format, document preamble); a dispatched L1 View's own `exportConfig` continues to govern its own render context (per-record preamble, `omitEmptyFields`) — two different concerns at two different granularities, not one value overriding the other. Rides the same `dataModelRevision: 7` stamp as srs#525's SectionSource → DiscoveryQuery collapse (one shared stamp per cutover, RFC-038/039 precedent). |
| 11 | 2026-09-19 | **Door 3 revision** (srs#827), executing the owner's ruling of 2026-09-19 on srs#794 (Option B, narrowed). Change E adds Rule **[N+38]**: when the per-record heading text a section resolves for a record equals that section's own title, the per-record heading MUST NOT be emitted and the record's rows render under the section heading. Deterministic, unconditional, no configuration and no new property. Rule [N+1]'s heading clause is amended in place with the revision marked, so the two rules cannot be read apart; RFC-020 Rule [N+37] is unchanged and [N+38] applies to whichever of the two resolved the text. The companion half of the same ruling is RFC-037 Revision 5 (`FieldView.labelMode`), which lands in the same pull request on RFC-037's own surface. No schema change and no `dataModelRevision` change; render is byte-unchanged in this PR, because the pinned CLI does not implement it — the implementation is the-greenman/srs-rust#1089. |

---

## Charter alignment (Revision 11 amendment, srs#827)

**Cell(s):** cell:conformance, cell:portability

**Decision mode:** complicated

**Governing cell preference:** Conformance — *one way over many* (`rfc-decision-cce3c00e`). Aligned. A duplicated heading is a defect with exactly one correct answer, so [N+38] is a rule and not a knob: no property is added, no View may opt out, and two conforming implementations given the same repository emit the same headings. Adding a switch would have created a second way to decide something the specification can decide once. Portability — *preserve over recognize* (`rfc-decision-cce3c00e`, `rfc-decision-8948e43f`). Aligned. The suppressed heading was never data: it is a projection of a field value the record keeps in full, still addressable and still present in every structured projection. Nothing is dropped from the stored form.

**Axis preference:** 5–11 Succession↔Conformance — default pole, **Reliability over Renewal**, with the boundary clause exercised as written: standing contracts hold, and renewal comes "only as explicit supersession at a declared boundary." [N+1] is amended in place with the revision marked at the point of change, in a numbered revision of the RFC that owns it, which is that boundary. 6–12 Containment↔Portability — default pole, Portability over Possession: the rule travels with the specification, not with an implementation's configuration. No non-default pole is taken.

**Decisions consulted:** `rfc-decision-0c403e15` (concepts describe themselves: a concept's introductory prose is the concept record's own prose, and "it renders at the concept's heading in every projection of the corpus") — the governing decision. A concept that renders at its own heading must not first render a second, identical one; today it does, on the opening page of this repository's own projection. `rfc-decision-8aed3412` (a story is built at EXPRESSION from reusable parts; the `contains`/`precedes`/`depends-on` tree is a claim about the records and no telling of them alters it) — [N+38] is a telling, not a claim. `rfc-decision-0118e938` (a construct has one layer) — the rule lives entirely in the presentation layer and reads only what that layer already resolved. `rfc-decision-92d2da05` (Composition → Presentation → Projection). `rfc-decision-4431046e` (a correction is not a refinement) — this changes the position [N+1] takes, so it is a marked amendment in a numbered revision, not a silent edit. `rfc-decision-7caca3a1` (decision modes), `rfc-decision-cce3c00e` (the grid and its preferences). RFC-020 Rule [N+37] was read and is untouched: it decides *what text* a heading carries, [N+38] decides *whether that heading is emitted*, and the two compose in that order.

**Contradictions found:** None. [T-6b] (structural headings arrive inside `{{content}}` and a wrapper MUST NOT re-emit them) is adjacent and is unaffected: [N+38] decides what the section pipeline puts *into* `{{content}}`, before any Theme sees it, and gives the Theme layer no new power.

**One-way-per-goal:** No second mechanism. Nothing in the specification today suppresses a redundant heading — this is the first and only rule for it, and it is deliberately not a configurable one, so no parallel path is created. `titleFieldId` absent is not an alternative: omitting `titleFieldId` suppresses *every* per-record heading in the section, which is a different goal and destroys the non-duplicating ones.

**Layer test:**
- **Which layer owns this?** EXPRESSION plane, presentation layer. The rule is evaluated by the section rendering pipeline over two strings that layer has already resolved — `DocumentSection.title`, and the per-record heading text from [N+1] or [N+37].
- **Consume or clone downward?** Consume. It re-implements no part of heading resolution; it reads the resolved text and decides whether to emit it. No Field, Type, Relation, record property or schema property is added.
- **Does the layer below stand alone without this?** Yes. Delete every Composition and the records, their fields and their relations are unchanged and still valid; [N+38] has nothing to read and nothing to suppress.
...
//   - The referenced field must satisfy the RFC-032 Rev-7 [N+1] facet predicate:
//     effective-single, datatype string, open valueDomain, format absent/plain/markdown.
...
### Change E — Suppress a per-record heading that repeats its section's title (Revision 11)

A `DocumentSection` carries a title, and Change B gives each record inside it a heading resolved from `titleFieldId` (or, under RFC-020 Rule [N+37], from the record's Type's effective `identityFieldId`). Nothing today notices when those two strings are the same. When they are, the reader gets the same words twice, one level apart, and the second heading carries no information at all.

This is not hypothetical and it is not an authoring error. It is the normal shape of a section whose subject *is* one record: a Part whose section title is the Part's name, containing the Part concept record whose own title is the same name. `rfc-decision-0c403e15` ruled that a concept's introductory prose is the concept record's own prose and "renders at the concept's heading in every projection of the corpus" — which is precisely the arrangement that produces the duplicate. The first thirty lines of `docs/spec/srs-spec.md` show it today.

**The rule.** When the per-record heading text a section resolves for a record is equal to that section's own title, the per-record heading is not emitted; the record's rows render directly under the section heading. It applies to every record in every section, in every covered format, with no property to set and nothing to opt into.

**Equality, stated precisely.** The comparison is exact string equality between the resolved per-record heading text and `DocumentSection.title`, each with leading and trailing whitespace trimmed — the same trim the baseline applies to a heading's text before emitting it, and the only normalisation either string receives. No case folding: *Reading this specification* and *Reading This Specification* are different headings and both are emitted. No internal whitespace collapsing, no punctuation folding, no Unicode normalisation beyond whatever the implementation already applies to both strings uniformly, and no prefix, suffix or substring matching. A section with no `title` has nothing to compare and emits every heading.

**Deliberately a rule, not a knob.** A duplicated heading has one correct treatment, so making it configurable would add a second way to decide a settled question — the Conformance cell's *one way over many*. The cost is stated rather than hidden: a View author who *wants* the same words twice at two depths cannot have them. No such author is known, and the escape is available without any new mechanism — change one of the two strings, which are separately authored.

**What it does not touch.** Only the heading is suppressed. The record's field rows, its relation rows, its wrapper and its classes render exactly as before, in the same order, at the same place. Heading *levels* are unchanged: a suppressed heading does not promote the rows to the section's level or shift `depthOffset`; the rows simply follow the section heading with no intervening one. The record itself is untouched — the field whose value produced the heading still carries that value, still validates, and still appears in every structured projection. And the rule reads only what the presentation layer has already resolved, so RFC-020 [N+37] is unchanged: [N+37] decides what text the heading would carry, [N+38] decides whether it is emitted, in that order.

#### Worked example

A section titled *Reading this specification*, containing one `concept` record whose `title` field — the section's `titleFieldId` — is also `Reading this specification`, followed by a second record titled *Purpose and Scope*.
...
**Description**: The nine Parts read front to back ...
...
The first heading is suppressed because it equals the section title. The second is emitted, unchanged and at the same level, because *Purpose and Scope* does not.
...
**Rule [N+1]** (ext:views-l2 — `titleFieldId` constraints; RFC-032 Rev-7 erratum): When `DocumentSection.titleFieldId` is set, it MUST reference a `fieldId` that (a) appears in the effective field list of every instance type in that section; (b) is **effective-single** (`fieldType.cardinality` absent/`"single"` and, until #242 Phase B, effective `FieldAssignment.repeatable !== true`); (c) has `fieldType.datatype == "string"`; (d) has `fieldType.valueDomain` absent or `"open"`; and (e) has `fieldType.format` absent or one of `"plain"` or `"markdown"`. Closed-domain, URI-, UUID-, and email-formatted strings, and every non-string or composite datatype, are ineligible. The section pipeline MUST NOT inject a structural per-record heading when `titleFieldId` is absent. This rule applies to the section rendering pipeline's heading injection mechanism only; it does not constrain headings that may appear inside `ExportConfig.preamble` content when an L1 View is bound via `renderViewId`. The section pipeline MUST NOT inject a structural per-record heading whose text equals the enclosing `DocumentSection`'s title, per Rule [N+38] **(Revision 11)**. This is enforced at render time; implementations SHOULD also enforce it at package validation time when the section source is statically determinable.
...
**Rule [N+38]** (ext:views-l2 — duplicate per-record heading suppression; RFC-001 Revision 11, srs#827): When the per-record heading text the section rendering pipeline resolves for a record — the value of the section's `titleFieldId` under Rule [N+1], or the record's Type's effective `identityFieldId` under RFC-020 Rule [N+37] — is equal to the enclosing `DocumentSection.title`, implementations MUST NOT emit that per-record heading; the record's rows MUST render directly under the section heading, at their existing levels and in their existing order. Equality is exact string equality after trimming leading and trailing whitespace from each string, which is the only normalisation applied to either; implementations MUST NOT case-fold, collapse internal whitespace, or match on prefix, suffix or substring. When `DocumentSection.title` is absent, this rule does not apply and the heading is emitted. Suppression affects the heading only: field rows, relation rows, wrapper elements, heading levels and `depthOffset` are unaffected, and the Record and its field values are unchanged. The rule is unconditional — no property enables, disables or overrides it. It is evaluated before any Theme is applied, so the suppressed heading never reaches `{{content}}` and `[T-6b]` is unaffected.
...
**Make [N+38] configurable, e.g. `DocumentSection.suppressDuplicateHeadings` (Revision 11)**: Rejected. A duplicated heading has one correct treatment, and a switch would create a second way to decide it — the Conformance cell's *one way over many*. An author who wants the same words at two depths can still have them by changing one of the two strings, which are separately authored.

**Fix the duplication by omitting `titleFieldId` on such sections (Revision 11)**: Rejected. `titleFieldId` is section-wide, so omitting it suppresses *every* per-record heading in the section, not the duplicated one. In a section whose first record duplicates the title and whose remaining records do not — which is the common shape — this trades one defect for a worse one.

**Compare loosely (case-insensitive, whitespace-collapsed, or by slug) (Revision 11)**: Rejected. A looser test suppresses headings the author meant to keep and makes the outcome depend on a normalisation the specification would then have to pin across formats and locales. Exact equality after a trim is decidable, implementable identically everywhere, and fails in the safe direction: an unsuppressed heading is visible and fixable, a wrongly suppressed one is silent.

...
| Rev | Date | Summary |
|---|---|---|
| 6 | 2026-08-25 | RFC-040 Change J (train unit 4a-5, srs#481, the #274 ratified ledger): `Field`, `Type`, and `Type.FieldAssignment` are **retired** from the Change B mapping table — their hand-authored ```typescript``` prose blocks in `records/subsections/04-2-4-2-field.json`/`04-3-4-3-type.json` are replaced by a `com.semanticops.spec/generated-type-reference` record (a typed generated-view slot) whose property table, optional pseudo-IDL, and raw-schema link are generated by `scripts/gen-type-reference-tables.mjs` from the same resolved effective Type the RFC-035 schema emitter projects the seed from — comparing two artifacts generated from one shared source cannot catch a defect that source doesn't already cause, so R1–R3 have nothing left to check for these three rows; the RFC-035/RFC-040 byte-closure tests are their real conformance gate now. Every allowlist entry naming one of the three retired rows retires with them (9 `Type.*` + 3 `Type.FieldAssignment.*`). **Open Question 1 is formally closed** (see below). Change D's allowlist entry shape gains a required `expected` field (the exact currently-observed mismatch shape) per `rfc-decision-5f8204bc`'s self-expiring-exception rule; `check-idl-schema-conformance.mjs` now fails loudly when a waived mismatch's shape *changes* or *disappears* (an entry that matched nothing this run is reported as stale), rather than silently waiving forever. Reconciling the remaining rows against current master retired five further entries whose named property no longer exists on either side (`Relation.sourceRepositoryId`/`targetRepositoryId`, `Container.containerId`/`rootInstanceIds`/`memberInstanceIds` — resolved by earlier, unrelated work and never swept up): the ledger shrinks from 30 entries to 13, all now self-expiring. |
...
|---|---|---|---|---|
| ~~Field~~ | **Retired (Rev 6, RFC-040 Change J, srs#481)** — generated, see below | — | — | — |
| ~~Type~~ | **Retired (Rev 6, RFC-040 Change J, srs#481)** — generated, see below | — | — | — |
| ~~Type.FieldAssignment~~ | **Retired (Rev 6, RFC-040 Change J, srs#481)** — generated, see below | — | — | — |
...
**Rev 6 retirement (RFC-040 Change J, srs#481):** `Field`, `Type`, and `Type.FieldAssignment` are no
longer checked by this mechanism at all. Their prose subsections
(`records/subsections/04-2-4-2-field.json`, `04-3-4-3-type.json`) no longer carry a hand-authored
```typescript``` block for these three rows — it was replaced by a
`com.semanticops.spec/generated-type-reference` record (a typed generated-view slot identifying the
metamodel Type by stable identity + version and a presentation profile), whose property table,
optional pseudo-IDL, and raw-schema link are generated by `scripts/gen-type-reference-tables.mjs`
from the *same resolved effective Type* the RFC-035 schema emitter (`scripts/lib/schema-emitter.mjs`)
projects `docs/schema/2.0/{field,type}.json` from. Comparing two artifacts independently generated
from one shared source can never catch a defect that source doesn't already cause — there is
nothing left for R1–R3 to usefully check for these three rows. Their real conformance gate is now
the RFC-035/RFC-040 byte-closure discipline (`rfc-035-closure-test.mjs`, the regenerate-and-diff
gate in `validate-all.mjs`) plus the reader-projection generator's own `--check` mode.
...
```json
{ "entity": "Term", "property": "lineage", "rule": "R1", "issue": 254, "expected": "prose-only" }
```
...
follow-up GitHub issue number. The script asserts at load time that every entry has all five
...
**`expected` (added Rev 6, RFC-040 Change J, srs#481 — `rfc-decision-5f8204bc`'s self-expiring-
exception rule):** a short string naming the *exact* currently-observed mismatch shape —
`"schema-only"` / `"prose-only"` for R1, `"idl=<required|optional>,schema=<required|optional>"`
for R2, or `"schema:type=…,format=…,enum=…"` for R3. At every run, the script computes the same
string for whatever mismatch it actually observes and compares it to the entry's `expected`: a
change (the mismatch moved to a different shape) is reported as loudly as an unwaived violation
would be, and an entry that matches *no* observed mismatch at all this run (the property was
resolved, removed, or the two sides now agree) is reported as stale at the end of the run. Before
Rev 6 an allowlist entry silently kept waiving forever even after its named condition changed or
disappeared — this reconciliation is what found `Relation.sourceRepositoryId`/`targetRepositoryId`
and `Container.containerId`/`rootInstanceIds`/`memberInstanceIds` already resolved and un-swept
(Rev 6's revision-history entry), which the same mechanism now prevents from recurring silently.
...
1. ~~Should `Type`'s nine extension-documented-as-comments properties eventually become real,
   ...
   property list?~~ **Formally closed (Rev 6, RFC-040 Change J, srs#481, the #274 ratified
   ledger, 2026-07-31):** neither option. `Type`'s hand-authored pseudo-IDL block is retired
   outright (see Change B's Rev 6 note) in favour of a *generated* property table whose
   **extension-owner column** names, per property, whether it is `core` or which `ext:<name>`
   contributed it — computed directly from the resolved effective Type (which facet Type
   declared each field), never hand-typed. The compact pseudo-IDL that survives as an optional
   appendix view gains **no ownership syntax** — that information lives in the property table,
   not duplicated into a second notation. This closes the question rather than answering it in
   either originally-proposed form: the pseudo-IDL block this question was about no longer
   exists as prose to extend.

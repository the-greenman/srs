...
| Rev | Date | Summary |
|---|---|---|
| 5 | 2026-08-09 | srs#242 Phase B cutover: the `Record.FieldValue -> record.json#/$defs/FieldValue` mapping row is dropped — RFC-039 [R7] deletes the `$def` and the entity ceases to exist; `check-idl-schema-conformance.mjs` no longer carries the row (RFC-039 Cross-references). |
...
| 4 | 2026-07-28 | Third review round (both reviewers re-run against Rev 3) found zero blocking findings — the RFC has converged. Small polish applied: the Note row's parenthetical path reference to the non-canonical `type-definitions/record-note.json` was missing its `srs/` prefix (fixed); Relation's "extra directionality-convention table" disagreement given the same "outside R1–R3 detection scope" caveat already applied to Container's case (both are free text after the fenced block, not in it); Container's "Invariant 19 vs 20" reframed from a cosmetic renumbering to what it actually is — the canonical copy's "19" is an objectively wrong citation to an unrelated invariant (`TypedField.name` uniqueness), the subsection copy's "20" is correct; Alt A blocker #3 and R3's sub-entity exclusion list updated to include `relationType`/`FieldValue`, which Rev 3 introduced elsewhere but hadn't fully threaded through every section referencing the same facts. Status → Accepted; `scripts/check-idl-schema-conformance.mjs` implemented and wired into `check-release-drift.mjs` per R5; the allowlist (Change D) seeded with every discrepancy the script actually reports at this commit, each citing a filed follow-up issue (srs#247–#254, srs-rust#777) per R6; reproduction proof against `origin/master` confirms it fires on the still-open #234/#235 and stays silent on the already-fixed #232, exactly as predicted. |

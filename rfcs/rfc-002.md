...
```typescript
{
  ...
  // For each fieldId listed, if the record contains an effective-single Field whose
  // fieldType is eligible under [T-9], the class
  // srs-field-{fieldName}-{normalisedValue} is added.
  // Only applies to "html" and "pdf" output. Ignored for other formats.
  // See [T-9] for the exact RFC-032 facet predicate; other Fields are silently skipped.
...
**Rule [T-9]** (ext:themes-l1 — `cssClassFields` scope; RFC-032 Rev-7 erratum): A Field listed in `Theme.cssClassFields` generates a CSS class only when it is **effective-single** (`fieldType.cardinality` absent/`"single"` and, until #242 Phase B, effective `FieldAssignment.repeatable !== true`), `fieldType.datatype == "string"`, and `fieldType.format` is absent or one of `"plain"` or `"markdown"`. `valueDomain` may be open or closed. URI-, UUID-, and email-formatted strings, list/repeatable Fields, and every non-string or composite datatype are silently skipped. Fields that are absent or empty on a given record generate no class for that record. This preserves scalar legacy `string`, `text`, and `select`, while the scalar requirement intentionally closes the previously undefined array-to-one-CSS-class case.

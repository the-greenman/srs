...
**Governing cell preference:** Conformance — *one way over many* (`rfc-decision-cce3c00e`). Aligned, and this is the reason `labelMode` is a single property in a single place. A field row's label is either emitted or it is not, and after this revision exactly one declaration decides which: `FieldView.labelMode`. No Theme path is opened ([T-3] and [FR-037-19] are untouched and [FR-037-22] states the closure explicitly), no sentinel value is given a second meaning, and no per-field-name template map is extended to reach top-level rows. Portability — *preserve over recognize* (`rfc-decision-cce3c00e`, `rfc-decision-8948e43f`). Aligned. The Record keeps every field it had; nothing is dropped from the stored form to make a document read better, and a repository whose Views are all deleted renders exactly what it rendered before this revision.
...
**Where a `FieldView` governs.** A `FieldView` is consulted where a View is bound to the rendering — `DocumentSection.renderViewId`, including the per-Type binding `typeDispatch` resolves ([N+14]–[N+18]). On the Default Rendering Baseline with no View bound there is no `FieldView` at all, so `"inline"` applies and output is unchanged. [FR-037-19]'s closing sentence — that `[T-10]`'s row form on the L1 View path is outside this RFC — is **not** amended and its subject is unchanged: this revision adds no rule about what row form a bound View emits. [FR-037-20] binds wherever the row being emitted is one of this RFC's forms, which in the reference implementation is both paths, because one emitter serves both (`render_service.rs::format_field_row`). Whether [FR-037-1] should be widened to cover the L1 View path's row form outright is a separate Door 3 question, recorded as Open Question 8 rather than answered here.
...
| `srs/records/tier-2/mechanism-e01a3121.json` (*Normative Field-Row Form (RFC-037)*) | **(Revision 5)** the `labelMode` row forms and `[FR-037-20]`–`[FR-037-22]` |
...
8. **Should `[FR-037-1]` cover the L1 View path's row form outright? (Revision 5)** `[FR-037-19]`'s closing sentence leaves it outside this RFC, which means a bound View's rows have no normative form even though the reference implementation emits this RFC's forms for them. Revision 5 does not settle it: `[FR-037-20]` binds wherever the row being emitted is one of this RFC's forms and says nothing about a row form this RFC never defined. Closing the gap properly would widen `[FR-037-1]` and is a Door 3 question of its own, not a rider on this ruling.

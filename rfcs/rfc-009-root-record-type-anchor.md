...
| 6 | 2026-08-28 | **Amended by srs#446** (the pattern-consistency audit's Membership signature, `rfc-decision-cce3c00e`, cell Containment "declaration over location"): replaces Rev 5's `rootInstanceIds[0]`-as-typing-anchor rule with an explicit, declared `Container.anchorInstanceId` field (the `identityInstanceId` pattern this RFC's own Container already carries via RFC-013, applied to the typing anchor). `rootInstanceIds`/`memberInstanceIds` regain the fully unordered-set semantics RFC-013 [R5] already claimed for them but Rev 5 quietly contradicted. Adds conformance rule I-145 and a transitional positional fallback for pre-#446 repositories, expiring at the Continuity flip (axis 2-8, `rfc-decision-cce3c00e`). Updates Change A's Resolution semantics, `blueprint.json:39`'s propagated `rootInstanceIds[0]` language, and RFC-010's merge semantics (Containers). Reconciles RFC-013 [R9] against RFC-038 [R12] (see RFC-013 Rev 7). |

---

## Abstract

The blueprint → view → container linkage is currently load-bearing on free-form strings: a Blueprint declares `name` ("guide"), a `DocumentView` declares `containerType: "guide"`, and a `Container` stores `containerType: "guide"`. Nothing validates these strings, nothing enforces referential integrity, a rename breaks the join silently, and namespace collisions are possible. This RFC adds `rootTypeRefs?: ExactTypeRef[]` to `DocumentView` as a validated, UUID-based typed anchor that replaces the string join. `containerType` is soft-deprecated to a validated hint; tools that predate this RFC may continue to use `containerType` string matching. This RFC also formally defines `Blueprint.rootTypes` as `ExactTypeRef[]` (the same UUID-based shape), completing the fully typed chain: `Blueprint.rootTypes → DocumentView.rootTypeRefs → Container.anchorInstanceId` — no string joins, and (Rev 6, srs#446) no positional reads, anywhere in this linkage. Additionally, this RFC blesses the `containers_for_instance` reverse lookup as a normative operation, and reconciles the Container metadata drift in the Rust implementation (`description` and `tags` are already present in the deployed struct but absent from the spec Container definition).
...
**Resolution semantics (amended by srs#446):** a DocumentView with `rootTypeRefs` present applies to a Container when the Container's typing anchor — the record named by `Container.anchorInstanceId` — has a `typeId`/`typeVersion` that matches one of the listed `ExactTypeRef` entries. **The typing anchor is `Container.anchorInstanceId`: an explicit, declared pointer, not a position.** When `anchorInstanceId` is absent (a repository authored before srs#446), an implementation MUST fall back to `rootInstanceIds[0]` (the first entry) as the typing anchor — transitional compatibility only; this fallback is withdrawn at the Continuity flip (`rfc-decision-cce3c00e` axis 2-8, the first full public release). Matching against any other entry in `rootInstanceIds`/`memberInstanceIds` for view selection is not performed. When no typing anchor can be resolved (`anchorInstanceId` absent, and `rootInstanceIds` **absent or empty**), no root-type restriction applies — the DocumentView applies to all Containers, the same as when `rootTypeRefs` is absent. Implementations SHOULD emit an informational diagnostic when `rootTypeRefs` is present but no typing anchor exists to validate against.

**Why this changed (srs#446).** Rev 5 stated the typing anchor as `rootInstanceIds[0]`, but RFC-013 [R5] independently states `rootInstanceIds`/`memberInstanceIds` are an unordered set with no positional significance. The two were in direct contradiction (pattern-consistency audit finding B3): one RFC relied on array position as a load-bearing typing signal, the other forbade deriving anything from array position. The Pattern Grid's Containment cell resolves it — "declaration over location" — by replacing the positional read with the explicit `anchorInstanceId` field, the same shape RFC-013's `identityInstanceId` already established for the container's identity pointer. `rootInstanceIds`/`memberInstanceIds` are now unordered without exception, for navigation (RFC-013 [R5]) and for typing alike.
...
>
> **[I-145]** *(Added by srs#446.)* When present on a Container, `anchorInstanceId` MUST equal an id contained in that Container's `rootInstanceIds` or `memberInstanceIds`; if it resolves to no such member, the repository is invalid. It names the record whose Type is the Container's typing anchor for I-63 matching and for RFC-010 three-way-merge conflict detection. When `anchorInstanceId` is absent, an implementation MUST fall back to `rootInstanceIds[0]` as the typing anchor (transitional; see Change A "Resolution semantics").
...
| `container.json` **(Rev 6, srs#446)** | Add optional `anchorInstanceId` (UUID) alongside `identityInstanceId`, same required-membership shape (I-145). Update `rootTypeRefs` cross-reference and `rootInstanceIds`/`memberInstanceIds` descriptions to state the array carries no positional significance for typing (RFC-013 [R5]), matching the `identityInstanceId` precedent. **Additive — no schema-required field, no `dataModelRevision` bump.** |
| `manifest.json` **(Rev 6, srs#446)** | Mirror the `container.json` `anchorInstanceId` addition into the embedded `$defs/Container` object (both Container schemas are closed — `additionalProperties: false` — so the field must be added to both, per the existing `identityInstanceId` precedent). |
| `blueprint.json` **(Rev 6, srs#446)** | Update `rootTypes` description (line ~39): the first entry's instanceId is set as `Container.anchorInstanceId`, not read from `Container.rootInstanceIds[0]`. Pre-#446 repositories keep the positional reading via the transitional fallback. |
...
**Note on spec records:** in addition to the JSON Schema files, the spec records in `srs/srs/records/` that describe the Container entity, the `ext:views-l2` extension, and the new invariants I-63 through I-66 must be created or updated when Stage 6 (authoring spec records) runs. (Rev 6: I-145 and the `04-6-4-6-container` subsection's illustrative shape, srs#446.)

**Migration — existing corpora (Rev 6, srs#446).** No repository authored before this revision carries `anchorInstanceId`; all rely on the retired positional rule. The transitional fallback (I-145) keeps every such repository conformant with no data change required. A repository MAY be migrated explicitly by setting each Container's `anchorInstanceId` to the id currently at `rootInstanceIds[0]`, once, per Container that has one — a pure derivation from existing data, reversible, and requiring no new instance, relation, or `dataModelRevision` bump. Actual execution of that migration over first-party corpora (`srs/srs`, `muSrs`, the gallery, package fixtures) is implementation-side follow-up, gated on the `srs-rust` anchor-support work this RFC's amendment requires (tracked in the srs#446 follow-up issue) — the pinned CLI in use as of this revision predates `anchorInstanceId` and would reject it as an undeclared property.

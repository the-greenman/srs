...
Tokens: `I-<n>`, `ext:<name>`, `schema:<file>.json`, `type:<ns>/<name>`, `section:<slug>`, `subsection:<slug>`, `concept:<slug>`, `mechanism:<slug>`, or `tooling-only` (no record/schema artifact — tooling/CLI/downstream-package only). `concept:`/`mechanism:` resolve against the RFC-042 concept-tree leaf Types that `section`/`subsection` retire into (srs#710) — a token written against a subsection that has since succeeded into a concept or mechanism must be updated to the new kind in the same change that retires the subsection. `scripts/check-rfc-integration.mjs` enforces that every accepted RFC declares a non-empty manifest whose tokens all resolve, and that the `.md` `**Status**:` line matches the record `rfc-status`. Genuinely-incomplete folds are grandfathered in `rfcs/integration-allowlist.json` with a follow-up issue. See `.claude/commands/rfc.md` Stage 6.
...
**`dataModelRevision` (RFC-033 / #265).** `manifest.json` and package manifests may carry an optional monotonic-integer `dataModelRevision` (absent ⇒ 0) stamping which data-model generation the data satisfies. RFC-032 (`valueType → fieldType`) is migration #1 → revision 1; srs#448 (Tier 1/TypedRecord removed, rfc-decision-53635966) is the migration to revision 4; srs#433 (substrate escape bag `properties → meta` on Term/RelationTypeDefinition/LifecycleState/LifecycleTransition — rfc-decision-6fc7e142, rfc-decision-628cf6c4) is the migration to revision 5; srs#524/#525 (Composition rename + `semanticObjectType` collapse, rfc-decision-92d2da05) is revision 6; srs#525 (ExportConfig unification) is revision 7; the spec repo is stamped `dataModelRevision: 7`. Not dotted semver.
...
- **Shallow-clone trap (`finding-07d153cf`, srs#668, srs#580 2026-09-09).** A worktree/clone can come back shallow. Against a shallow history, any git-history investigation (`git log -- <path>`, `git cat-file -t <sha>`, `git merge-base --is-ancestor`, and `check-versioning-cell.mjs`) can silently report a real, live commit or violation as absent — no error, just a confidently wrong answer. This has produced a false "stale allowlist entry" diagnosis on `srs#658` at least five times across two days. Before trusting any such result: `git rev-parse --is-shallow-repository`; if `true`, `git fetch --unshallow` first.
...
- **Revision-bump choreography, four steps in order:** (1) srs-rust support lands — supported-revision constant + migration-registry entry + fixture test; binary loads old AND new revisions; (2) release cuts with the corpus gate green; (3) the spec-side PR merges; (4) ONE pin advance — srs `SRS_RUST_CLI_TAG` in `.github/workflows/release-drift.yml` re-rendered in the same PR; srs-web `scripts/ensure-bindings.mjs`; muDemocracy.org `scripts/ensure-srs-cli.mjs`. Rev 7 is the last revision landed as of this writing (the pinned `SRS_RUST_CLI_TAG` build advances independently of revision bumps — see `.github/workflows/release-drift.yml` for the current pin, not this line).
- **Merge rights are earned by classification, not assumed.** Every PR body states its **decision mode** (`rfc-decision-7caca3a1`) and **which door** (below). An agent may merge only when the mode is clear or complicated AND the change is Door 1 (executes a recorded ruling, citing its `rfc-decision-*` id) or non-normative (tooling, checks, docs, `programme/`) — label `gate:auto-merge` and use `gh pr merge --auto`. Mode complex, Door 2, or Door 3 carries `gate:owner-merge` and the agent never merges. Mode chaotic stops. This is enforced, not promised: `validate-all` and `check-release-drift` are required contexts on master, strict, zero reviews — a red PR with auto-merge armed does not merge (proven, #577). Conflicts are resolved by grafting complete blocks (functions + helpers + call sites), never whole-file ours/theirs.

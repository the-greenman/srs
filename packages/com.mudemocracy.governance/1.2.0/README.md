# com.mudemocracy.governance @1.2.0

**v1.2.0** (srs rev-6 composition-cutover, srs#523/#524): mechanical rename only — no view's
shape changed. `document-views/` → `compositions/`, `$schema` repointed to `composition.json`,
and the type-query `SectionSource`'s `semanticObjectType` key renamed to `typeKey` (values
unchanged — all four views already used KEYED `namespace/name` form, e.g. `governance/article`).
`package.json`'s `documentViews` manifest key is now `compositions`. See `package/package.json`'s
